...
\emph{Hypothesis-level gap (why this is a sketch, not a proof).}
The composition above maps each discrete condition (A1)--(A8)
to the classical hypothesis it is intended to discharge, but it
does \emph{not} verify that the discrete conditions are
\emph{exactly} the hypotheses the cited theorems require --- in
particular, Sturm's curvature-dimension condition $CD(K,N)$ and
the Lott--Villani measure-contraction property are not
explicitly shown to be implied by (A5)+(A6). Promoting this
result from ``[Sketch]'' to a full ``[Proof]'' would require an
explicit hypothesis-level table establishing each such
implication; until then the theorem is used only in the
conditionally-rigorous sense made explicit in
Corollary~\ref{cor:cbi_admissibility}.
\end{proof}
