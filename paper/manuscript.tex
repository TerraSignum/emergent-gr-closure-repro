...
graphs. Four candidate analytical routes
...
\path|notes/lemma_B_proof_strategy.md|, which recommends the
radial spectral-synthesis route as primary: the carrier-action
UV-closure forces $\xi_{N}(i,j)\!=\!f(d_{N}(i,j))$ for a single
bounded monotone $f$, so $P_{N}\!=\!D_{N}^{-1}W_{N}$ is radial
on the carrier metric and admits a closed-form spectral
expansion whose leading term should reproduce the conjectured
$\lambda_{\infty}\!=\!3/8$ of Proposition~\ref{prop:sg_empirical}.
