...
\newcommand{\repoOwn}{emergent-gr-closure-repro}
...
$G_{\mathrm{eff}}/G_{N} = 0.9998$ (a calibration-reproduction
consistency check, not a parameter-free closure) and emergent
spectral dimension $d_{\mathrm{eff}}=3.170$; the Bekenstein--Hawking area law
...
grid (\pyown{src/stage6h_nonlinear_xi_eom_static.py}{stage6h_nonlinear_xi_eom_static}). The maximum
...
\pyown{src/stage6b_solve_4_problems.py}{stage6b_solve_4_problems}.  The
...
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_subleading_finite_N_corrections.py}{verify_subleading_finite_N_corrections} and
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_beta_pi_multi_level.py}{verify_beta_pi_multi_level}. A multi-Ebene scan
...
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_two_loop_corrections.py}{verify_two_loop_corrections}).
...
\!\approx\!(0.105,\, 0.530,\, 0.785)$, with sub-percent
...
Reproducers:
\pyscript{causal-wave-landings-repro}{src/verify_dynamic_coefficient_flip.py}{verify_dynamic_coefficient_flip},
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_symanzik_continuum_anchors.py}{verify_symanzik_continuum_anchors},
\pyscript{causal-wave-landings-repro}{src/verify_chirality_running_detailed.py}{verify_chirality_running_detailed},
\pyscript{causal-wave-landings-repro}{src/verify_chirality_refined_rationals.py}{verify_chirality_refined_rationals};
...
SM observables (e.g.\ the legacy $|V_{us}|^{(M)}\!=\!\gamma^{(M)}\sqrt{5}\!=\!2.01$
running-coupling diagnostic --- the canonical closure is the post-supersession
$V_{us}\!=\!\alpha_{\xi}\!\cdot\!s_{\rm face}\!=\!9/40$ of Paper~3 ---
...
\pyscript{causal-wave-landings-repro}{src/verify_post_flip_composition.py}{verify_post_flip_composition}).
...
m_{3}^{(\nu)} &= m_{e}\!\cdot\!\gamma^{d+3}\!\left(1\!-\!\tfrac{\gamma^{2}(d{+}N_{\rm gen})}{4}\right)
              = 0.05023\,\text{eV} \nonumber \\[2pt]
&\quad \text{vs } \sqrt{\Delta m_{31}^{2}} = 0.0502\,\text{eV}
\quad (0.068\%,\text{ EXACT)}
\label{eq:m3_post_flip} \\[6pt]
\Sigma m_{\nu}^{\rm NH,\,min} &= 0.05883\,\text{eV}
\quad (\text{EXACT $0.05\%$})
\label{eq:Sigma_mnu_post_flip}
\end{align}
The references for these reductions are NuFIT 6.1 + companion
P4B Eq.~p4b\_m3\_nu\_dressed for $m_{3}$, and DESI DR2
$\Sigma m_{\nu}<0.0642\,$eV~\cite{DESI2025} for the sum.
...
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_eta_B_precise_form.py}{verify_eta_B_precise_form},
...
(reproducers
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_neutrino_mass_DESI_2024.py}{verify_neutrino_mass_DESI_2024},
...
(e.g.\ $|V_{us}|$ predicted via the legacy form $\gamma\sqrt{5}$,
superseded by $\alpha_\xi\!\cdot\!s_{\rm face}\!=\!9/40$ in
Paper~3~\cite{Paper3}, would still go from
$0.224$ at $N\!=\!50$ to $2.14$ at $N\!=\!1000$ under the
running-coupling diagnostic).
...
(reproducer
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_chirality_flip_implications_existing_closures.py}{verify_chirality_flip_implications_existing_closures}).
...
\pyscript{causal-wave-landings-repro}{src/verify_clifford_projector_derivation.py}{verify_clifford_projector_derivation}; $0.60\%$
...
\pyscript{causal-wave-landings-repro}{src/verify_fast_slow_D_Omega_predictions.py}{verify_fast_slow_D_Omega_predictions}.
...
(reproducers
\pyscript{causal-wave-landings-repro}{src/verify_D_Omega_lattice_resonance.py}{verify_D_Omega_lattice_resonance},
\pyscript{causal-wave-landings-repro}{src/verify_C2_violation_structural_cause.py}{verify_C2_violation_structural_cause}).
...
the Cabibbo-class generation count appearing in the legacy
reading $|V_{us}|\!=\!\gamma\sqrt{5}$, now superseded by
$\alpha_{\xi}\!\cdot\!s_{\rm face}\!=\!9/40$ in
Paper~3~\cite{Paper3}). The C$_{2}$ identity
...
\pyscript{causal-wave-landings-repro}{src/verify_D_Omega_alternative_derivations.py}{verify_D_Omega_alternative_derivations}.
...
(matter-saturation asymptote, see Fig.~\ref{fig:chirality_higher_N}).
...
Reproducers:
\pyscript{causal-wave-landings-repro}{src/verify_chirality_running_first_principles.py}{verify_chirality_running_first_principles},
figure: \path|paper/figures/fig_chirality_higher_N_predictions.pdf|
(see Fig.~\ref{fig:chirality_higher_N}).

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.97\textwidth]{figures/fig_chirality_higher_N_predictions.pdf}
...
number of named statements. A reader pressed for time can
...
& $G_{\rm eff}/G_{N}=0.999791$, residual $0.021\%$ (consistency
check: numerical reproduction of the calibrated $G_{N}$, not a
parameter-free closure --- the far-field \emph{form}
$1-2GM/r$ is derived, the prefactor is calibrated to $G_{N}$,
see~\cite{Paper4A})
...
\pyown{src/audit_M3_violations.py}{audit_M3_violations} reports three quantile-style
...
(\pyown{src/audit_M3_dynamic_causal.py}{audit_M3_dynamic_causal}, output
...
(\pyown{src/verify_sup_aware_envelope_refinement.py}{verify_sup_aware_envelope_refinement}; output
...
(\pyown{src/audit_M3_defect_quantitative.py}{audit_M3_defect_quantitative}, output
...
emergent time direction, $d_{\mathrm{spacetime}} = 4$. The
emergence of the Lorentzian signature from a curvature-minimising
relational geometry --- the equilibrium-field eigenvalue pattern
fixing the local causal structure --- is a recognised
mechanism~\cite{Bermudez2025}, of which the present
Einstein-residual closure is a concrete relational-lattice
...
(\pyown{src/stage6h_nonlinear_xi_eom_static.py}{stage6h_nonlinear_xi_eom_static};
...
\pyown{src/verify_galerkin_schwarzschild_defect_gpu.py}{verify_galerkin_schwarzschild_defect_gpu} then
...
The far-field \emph{form} $\Xi(r)=1-2GM/r$ follows
parameter-free from the linearised $\Xi$-EOM; the $1/r$
prefactor is then identified as $G$ by calibration to $G_{N}$
(see~\cite{Paper4A}). With that calibration the Galerkin
pipeline reproduces $G_{\mathrm{eff}}/G_{N} = 0.999791$ at the
canonical regime --- a residual of $0.021\%$, which is a
\emph{numerical-reproduction consistency check} of the
calibrated constant, not a parameter-free $\mathcal R$-rational
closure --- as recorded in the bundled black-hole data file. The same Schwarzschild defect feeds the
...
\end{tabular}
\par\medskip
\footnotesize $^{\dagger}$ CLP-B/B4 carries the per-sub-component
...
A bundled architectural-ceiling audit
(\pyown{src/verify_clp_b_architectural_ceiling.py}{verify_clp_b_architectural_ceiling},
\path|outputs/verify_clp_b_architectural_ceiling.json|)
identifies the six inner asymptotes feeding the absorption
sub-score as clean System-R rationals (support\_score $5/6$,
support\_persistence $49/60$, macro/intrinsic/structural/calibration
distances $13/25,\,9/32,\,3/8,\,5/12$) and shows the absorption
asymptote is architecturally bounded above by
$\tfrac{2(5/6)(49/60)/(1+\tfrac{1}{4}(13/25+9/32+3/8+5/12)) + 0.652 + 0.686}{4}\!\approx\!0.578$,
so that $0.70$ is unreachable without redefining the
\texttt{quality\_seed} primitive and $0.50$ is the natural —
not heuristic — sub-component threshold.
\end{table}
...
Two clean large-$N$ lattice sizes are bundled with the
manuscript:
...
$\langle K\rangle(\theta)\!=\!K_{\rm pre}\cos^{2}\theta
\!+\!K_{\rm post}\sin^{2}\theta
\!+\!a_{K}\sin(2\theta)\!+\!b_{K}\sin(4\theta)$
(\cite{Paper4B}~Eq.~13 (KQ-full-closure), with the leading
$\sin(2\theta)/\sin(4\theta)$ endpoint-preserving Fourier
harmonics added to the basic
$\cos^{2}/\sin^{2}$ interpolation) interpolates between the
pure-vacuum ($\theta\!=\!0$) endpoint
$K_{\rm pre}\!=\!4/3-\gamma^{2}/3\!=\!133/100$ and the
pure-matter ($\theta\!=\!\pi/2$) endpoint
$K_{\rm post}\!=\!4/3+\gamma^{3}$, with
$K_{\rm post}\!-\!K_{\rm pre}\!=\!\gamma^{2}/3+\gamma^{3}\!=\!13/3000$;
the analogous form for $Q$ uses
$Q_{\rm pre}\!=\!1/4+\gamma^{2}(N_{\rm gen}{+}d)\!=\!8/25$,
$Q_{\rm post}\!=\!1/4+\gamma^{2}N_{\rm gen}/d^{2}\!=\!403/1600$.
Branch-resolved empirical means on the same N-split:
...
\pyown{src/verify_chirality_flip_cross_sector.py}{verify_chirality_flip_cross_sector}, output
...
(\pyown{src/audit_omega_m_dual_form_corpus_anchors.py}{audit_omega_m_dual_form_corpus_anchors}).}
...
(ii) PDG~2024 cosmology compact~\cite{PDG},
...
Reproducer: \pyown{src/verify_chirality_flip_cross_sector.py}{verify_chirality_flip_cross_sector},
...
five-point PRE-flip ladder; what is rejected here is the
\emph{single-branch isotropic} hypothesis at the corresponding
trace value, not the rational $161/200$ as a numerical residue
(the value reappears as a half-coefficient neighbour of the
branch-resolved family below). The isotropic-positive $1{+}3$
form $\alpha_{\xi}^{2}\!+\!3\gamma^{2}/2\!=\!33/40$ is the
canonical \emph{vacuum-branch} $\Lambda_{t}$ per
Eq.~\ref{eq:lambda_t_branch_resolved} and is in the 5-point
PRE-flip CI; what is rejected on the seven-point mixed ladder
is the single-branch isotropic reading at this trace value
(the value $33/40$ survives as the canonical vacuum-branch
trace in the branch-resolved family). The remaining four candidates
...
regularisation, Coleman--Gamow vacuum sequestering --- the
lattice analogue of vacuum-energy
sequestering~\cite{KaloperPadilla2014} ---, spectral-
...
Planck~2018~\cite{Planck2018} ($\Omega_{\Lambda} = 0.6847\pm
...
$w_{0} = -1.03\pm 0.03$ (Planck+SNe+BAO~\cite{Planck2018}),
...
precision-cosmology test. Planck~2018~\cite{Planck2018}
...
recorded on the canonical-physics ladder. The
...
The closest external precedent for a $\Lambda$ value selected
by a discrete spacetime measure (rather than tuned by an
effective action) is the causal-set everpresent-$\Lambda$
construction~\cite{Surya2024fluctuations,Surya2019}, which
predicts $\Lambda\!\sim\!1/\sqrt{V_{\rm spacetime}}$ as a
continuous fluctuating-discrete prediction; the present
framework's small-integer-rational selection $19/15$, by
contrast, is a discrete-measure prediction with the specific
two-component decomposition $17/20+5/12$ above, so the two
constructions agree on the \emph{class} of mechanism (discrete
measure selects $\Lambda$) but disagree on the \emph{form}
(continuous fluctuation vs. small-integer-rational selection).

\emph{Physical-reading proposal for the integer pair
$(19,15)$.} Both the numerator and denominator factorise into
framework primitives:
$15\!=\!(d{+}1)\,N_{\rm gen}\!=\!5\!\cdot\!3$ is the
\emph{sync-extended cone bundle} (the $d{+}1$ spatial-plus-
sync-axis count $\times$ the three generations), and
$19\!=\!4d{+}N_{\rm gen}\!=\!16{+}3$ is the
\emph{chirality-spin DoF plus generation content}
(chirality-doubled $\times$ spin-doubled spatial DoF
$4d\!=\!16$, augmented by the generation count $N_{\rm gen}\!=\!3$).
The compact additive form
\begin{equation}
\label{eq:Lambda_lat_structural_proposal}
\Lambda_{\rm lat}^{\infty}
   \;=\;\frac{4d+N_{\rm gen}}{(d{+}1)\,N_{\rm gen}}
   \;=\;1\;+\;\frac{N_{\rm gen}+1}{(d{+}1)\,N_{\rm gen}}
   \;=\;1\;+\;\frac{4}{15}
\end{equation}
reads $\Lambda_{\rm lat}$ as
\emph{unity (the trivial carrier-baseline cosmological weight)
plus the per-generation chirality-spin defect distributed over
the full sync-extended cone bundle}: the $N_{\rm gen}\!+\!1\!=\!4$
in the additive correction is the chirality-doubled-generation
content (three generations $+$ one conjugate-spin partner that
the (SG)-axiom $\lambda_{\infty}\!=\!(d{-}1)/(2d)\!=\!3/8$
selects on the matter branch), and the $(d{+}1)\,N_{\rm gen}\!=\!15$
in the denominator is the total cone-bundle DoF count over
which this defect is distributed. The two-component algebraic
decomposition $17/20\!+\!5/12$ used in the principal closure
above is then the operator-side projection of the same
identity onto the Clifford-channel + spinor-trace basis;
the additive $1\!+\!4/15$ reading is the carrier-side
\emph{cone-bundle} projection. The two readings are equivalent
by direct arithmetic ($1\!+\!4/15\!=\!19/15\!=\!17/20\!+\!5/12$),
which is the structural reason the lattice cosmological
constant is bounded away from the trivial unit value by the
small-rational defect $4/15$ rather than by a continuous-
spectrum quantity. This proposal is \emph{not} a theorem; it
is a physical-reading conjecture motivating the open
``mechanism\_open'' classification.
...
(\textit{PDG 2024 Review, Lattice QCD chapter~17}~\cite{PDG}), residuals
...
of $0.028$ remains below the threshold. A regime-invariance test on the canonical-physics ladder
...
\emph{Ladder dependence.} This $0.81811$ is the within-$P_5$
$8$-point extrapolation, which sits $1.0\%$ \emph{above}
$\alpha_{\xi}^{2}$; the wider $10$-point snapshot ladder
$N\!\in\![50,512]$ extrapolates instead to $0.80361$, which
sits $0.8\%$ \emph{below} $\alpha_{\xi}^{2}$ and matches the
$\gamma^{2}$-shifted rational
$\alpha_{\xi}^{2}(1\!-\!\gamma^{2})=8019/10000$
(Remark~\ref{rmk:t00_summand_decomposition}). The two ladders
therefore \emph{bracket} $\alpha_{\xi}^{2}$, and whether the
true continuum value is $\alpha_{\xi}^{2}$ or
$\alpha_{\xi}^{2}(1\!-\!\gamma^{2})$ is not resolved at the
present resolution. As shown next, the CBI ratio conclusion is
invariant under this ambiguity.
\end{lemma}
...
\begin{remark}[Per-summand decomposition of the $T_{00}^{\Xi}$
asymptote]
\label{rmk:t00_summand_decomposition}
The four-term structure of the per-node $T_{00}$ scalar admits a
\emph{per-summand} Symanzik extrapolation, instrumented by the
bundled reproducer
\pyown{src/verify_t00_summand_decomposition.py}{verify_t00_summand_decomposition} (output
\path|outputs/t00_summand_decomposition_audit.json|) on the
canonical ladder $N\!\in\![50,512]$. Rather than extrapolating
the noisy aggregate, each summand is extrapolated separately,
and each lands within seed-bootstrap $95\%$ confidence on a
clean System-$\mathcal R$ rational: the edge-$\Xi$ row-variance
term $\tfrac12\sigma^{2}_{\Xi}\!\to\!\alpha_{\xi}^{2}\gamma^{2}$,
the amplitude-variance and gradient-energy terms vanish
($\sigma^{2}_{|\psi|},|\nabla\psi|^{2}\!\to\!0$), and the $K/Q$-recoil
term $\to\!\alpha_{\xi}^{2}(1\!-\!2\gamma^{2})$. Their exact sum
is $\alpha_{\xi}^{2}(1\!-\!\gamma^{2})=8019/10000$, and this
value --- not $\alpha_{\xi}^{2}$ itself --- is the one that lies
inside the bootstrap CI of the \emph{independent} direct
$T_{00}^{\Xi}$ Symanzik fit. The decomposition is therefore
internally consistent, and it shows that the sub-percent offset
disclosed above is \emph{structured}: it is the $\gamma^{2}$
correction $\alpha_{\xi}^{2}\!\to\!\alpha_{\xi}^{2}(1\!-\!\gamma^{2})$,
not unstructured extrapolation noise. Whether the true continuum
value is $\alpha_{\xi}^{2}$ (the finite-$N$ data still rising) or
$\alpha_{\xi}^{2}(1\!-\!\gamma^{2})$ is thereby sharpened into a
single, decidable question. The leading row-variance identity
$\sigma^{2}_{\Xi}\!\to\!2\alpha_{\xi}^{2}\gamma^{2}=81/5000$ is
independently re-certified by
\pyown{src/verify_variance_reactivity_identity.py}{verify_variance_reactivity_identity}, so the
variance-reactivity coefficient $\alpha_{\xi}$ enters $T_{00}$
algebraically rather than as a label
(Figure~\ref{fig:t00_summand_decomposition}).

\emph{Branch-resolved sharpening of the $S_{4}$ bottleneck.}
The pooled $R^{2}\!=\!0.50$ of $S_{4}$ in the per-summand
decomposition above is sharpened by a branch-resolved
re-aggregation of the bundled per-seed data
(\pyown{src/verify_t00_summand_branch_resolved.py}{verify_t00_summand_branch_resolved}, output
\path|outputs/verify_t00_summand_branch_resolved.json|),
together with K/Q-closure-derived target candidates from
$0.5\langle K\rangle\!+\!0.25\!-\!0.25\langle Q\rangle$ evaluated
at the chirality-mixing endpoints
$(K_{\rm pre},Q_{\rm pre})\!=\!(133/100, 8/25)$ and
$(K_{\rm post},Q_{\rm post})\!=\!(4/3+\gamma^{3}, 403/1600)$
from \cite{Paper4B}~Eqs.~(KQ-full-closure)
and the Clifford-algebra-anchored target
$1-N_{\rm gen}/2^{d}$. The canonical ladder $N\!\in\![50,512]$
spans the chirality-flip at $N_{*}\!\approx\!110$--$120$;
branch-separated Symanzik-1 fits on the regime-median ladder
expose two distinct structural identifications for $S_{4}$:
\begin{align}
\label{eq:S4_branch_asymptotes}
S_{4}^{\rm mat,\infty} &\;\to\; 0.81027
\quad\bigl(\alpha_{\xi}^{2}\!=\!81/100,\;\text{residual }-0.03\%,\text{ EXACT}\bigr),
\nonumber\\
S_{4}^{\rm vac,\infty} &\;\to\; 0.81376
\quad\bigl(\tfrac{13}{16}\!=\!1-N_{\rm gen}/2^{d},\;\text{residual }-0.15\%,\text{ PRECISE}\bigr).
\end{align}
The matter branch is essentially converged ($S_{4}$ flat across
$N\!\in\![128,512]$), so its Symanzik $R^{2}\!=\!0.26$ reflects
absence of finite-$N$ variation to explain, not poor asymptotic
identification; the matter-branch asymptote sits at EXACT tier
on $\alpha_{\xi}^{2}$. The vacuum branch retains finite-$N$
curvature ($R^{2}\!=\!0.62$) and identifies a distinct
Clifford-algebra-anchored target
$13/16\!=\!1\!-\!N_{\rm gen}/2^{d}$ at PRECISE tier --- the
\emph{same} Clifford-algebra-dimension denominator $2^{d}\!=\!\mathrm{Cl}(d)$
that appears in the curvature-dimension cross-projection
$K_{p_{5}}^{\rm BE}\!=\!\lambda_{\infty}N_{\rm gen}(d{+}N_{\rm gen})/2^{d}$
(par.~\ref{par:cd_cross_projection}), so the
chirality-doubled-spatial Clifford structure governs both the
Bakry--\'Emery curvature distribution and the vacuum-branch
$K/Q$-recoil response of $T_{00}$. The pooled $R^{2}\!=\!0.50$ is
therefore not a fit deficiency but a chirality-flip-bracketing
artefact: the per-branch identifications
$S_{4}^{\rm mat}\!\to\!\alpha_{\xi}^{2}$ (EXACT) and
$S_{4}^{\rm vac}\!\to\!1\!-\!N_{\rm gen}/2^{d}$ (PRECISE) are
each individually cleaner than the pooled
$\alpha_{\xi}^{2}(1\!-\!2\gamma^{2})$ reading.

\emph{Status of the per-summand decomposition.} The
$T_{00}^{\Xi}$ continuum limit is no longer characterised
only at audit level: the three structural lemmas
\textsc{RV}, \textsc{AG}, and \textsc{KQ-recoil} stated
immediately after this remark close
$T_{00}^{\Xi}\!\to\!\alpha_{\xi}^{2}(1-\gamma^{2})$ at
\emph{conditional theorem level} (Theorem~\ref{thm:t00_continuum},
conditional on $(SG)$+admissibility plus the P4B chirality-mixing
closure).
\end{remark}
...
{figures/fig17_t00_summand_decomposition.pdf}
\caption{Per-summand continuum decomposition of $T_{00}^{\Xi}$
on the canonical-physics ladder $N\!\in\![50,512]$
(\texttt{src/verify\_t00\_summand\_decomposition.py}).
\textbf{(a)} The noisy aggregate $T_{00}$ and the dominant
$K/Q$-recoil summand $S_{4}$, with the direct-fit bootstrap
$95\%$ CI band (grey); the structural sum
$S_{1}{+}S_{2}{+}S_{3}{+}S_{4}=8019/10000$ (blue star) lies
inside that band, on $\alpha_{\xi}^{2}(1\!-\!\gamma^{2})$ (blue
dotted), while $\alpha_{\xi}^{2}=81/100$ (red dashed) sits above
it. \textbf{(b)} The three sub-leading summands: the edge-$\Xi$
row-variance term $S_{1}\!=\!\tfrac12\sigma^{2}_{\Xi}$ lands on
$\alpha_{\xi}^{2}\gamma^{2}=81/10^{4}$ while the amplitude-variance
and gradient-energy terms $S_{2},S_{3}$ vanish. Right-pointing
markers are the per-summand Symanzik asymptotes.}
\label{fig:t00_summand_decomposition}
\end{figure}

\paragraph{From audit to theorem-conditional: three structural
lemmas underlying the per-summand decomposition.}
The four per-summand identifications above are upgraded from
``Symanzik fit lands on a System-$\mathcal{R}$ rational'' to
\emph{conditionally-rigorous lemmas} by separating the
structural argument for each summand from the empirical
certificate. The three load-bearing lemmas together close the
$T_{00}^{\Xi}$ continuum-limit budget; the closure rests on the
$(SG)$ axiom plus the K/Q chirality-mixing closure of
P4B~\cite{Paper4B}~Eq.~KQ-full-closure.

\begin{lemma}[Row-variance, RV]\label{lem:row_variance_RV}
Under the uniform spectral-gap axiom $(SG)$
($\lambda_{\infty}=3/8$) plus admissibility $A_{1}$--$A_{8}$ on
the carrier path and the synchronisation-locking
$\varepsilon^{2}_{\rm sync}=\gamma/2$ of P2~\cite{Paper2},
the per-node row-variance of the $\Xi$-matrix in the stationary
distribution of the fast-slow generator obeys
\begin{equation}
\sigma^{2}_{\Xi}(N) \;\xrightarrow[N\to\infty]{}\;
2\,\alpha_{\xi}^{2}\,\gamma^{2} \;=\; \frac{81}{5000}.
\label{eq:lemma_RV}
\end{equation}
\end{lemma}

\begin{proof}[Sketch]
\emph{(A1) Stationary distribution.} Under $(SG)$ and the
three Bakry--\'Emery curvature-dimension bounds
$K_{\rm BE}\!\in\!\{15/32,\,63/128,\,51/88\}$ certified in
par.~\ref{par:cd_cross_projection}, the slow generator
$\partial_{t}\Xi_{ij}\!=\!-(\partial S_{\rm UV}/\partial\Xi_{ij})$
admits a unique stationary distribution $\rho_{\rm eq}$ with
finite second moment (Sturm~\cite{Sturm2006},
Lott--Villani~\cite{LottVillani2009}).
\emph{(A2) Variance--reactivity (Fluctuation--Dissipation).}
In the near-equilibrium linearisation around $\rho_{\rm eq}$,
$\sigma_{\Xi}^{2} = 2\chi\,D_{\varepsilon}$, where $\chi$ is the
linear susceptibility of $\Xi_{ij}$ to a source coupled in the
slow direction and $D_{\varepsilon}$ is the noise strength of
the fast sector.
\emph{(A3) Algebraic identification.} The susceptibility
decomposes onto the four bounded-operator projectors of
P2~\cite{Paper2}; the dominant block carries eigenvalue
$\chi=\alpha_{\xi}^{2}$ (squared because variance is a second
moment). The fast-sector noise strength is the slow/fast
separation parameter $D_{\varepsilon}=\gamma^{2}$ inherited
from the chirality-flip angle squared (P4 Sec.~$D_{\Omega}$).
Combining gives $\sigma_{\Xi}^{2}\!\to\!2\alpha_{\xi}^{2}\gamma^{2}$.
\qed
\end{proof}

\noindent\emph{Empirical certificate.}
\pyown{src/derive_row_variance_lemma.py}{derive_row_variance_lemma}
loads the per-summand Symanzik audit, confirms
$S_{1}=\tfrac12\sigma_{\Xi}^{2}$ asymptotes to
$0.00805\!\pm\!0.00046$ ($\rm CI95$ from seed-resampling
bootstrap), and contains the rational target
$\alpha_{\xi}^{2}\gamma^{2}=81/10^{4}=0.00810$ at relative
residual $-0.60\%$. The independent variance--reactivity
audit of \pyown{src/verify_variance_reactivity_identity.py}{verify_variance_reactivity_identity}
returns verdict \texttt{VARIANCE\_REACTIVITY\_IDENTITY\_CONFIRMED};
the bundled artifact is
\path|outputs/derive_row_variance_lemma.json|.

\begin{lemma}[Amplitude-/gradient-vanishing, AG]\label{lem:amp_grad_AG}
Under the same hypotheses $(SG)+A_{1}\!-\!A_{8}+$\,sync-lock,
the amplitude variance and gradient energy of the per-node
wavefunction vanish in the continuum limit:
\begin{equation}
\sigma^{2}_{|\psi|}(N)\;\xrightarrow[N\to\infty]{}\;0,
\qquad
|\nabla\psi|^{2}(N)\;\xrightarrow[N\to\infty]{}\;0.
\label{eq:lemma_AG}
\end{equation}
\end{lemma}

\begin{proof}[Sketch]
\emph{(B1)} The Poincar\'e inequality on the $\Xi$-weighted
normalised Laplacian,
$\langle(f-\langle f\rangle)^{2}\rangle\le
\lambda_{2}^{-1}\langle|\nabla f|^{2}\rangle$, with
$\lambda_{2}\!\to\!\lambda_{\infty}\!=\!3/8$ under $(SG)$.
\emph{(B2) Sync-lock.} The synchronisation block-projector
$\Pi_{\rm sync}$ of P2~\cite{Paper2} regulates $|\psi|$ above
the sync scale $N\!\ge\!N_{\rm sync}$ with
$\sigma_{|\psi|}^{2}(N)\!\le\!\varepsilon^{2}_{\rm sync}\langle|\psi|\rangle^{2}
\,f(N)$ and $f(N)\!\to\!0$ at the spectral-decay-of-non-synchronised-modes rate.
\emph{(B3) Cheeger--Buser.} Eq.~(B1) implies
$|\nabla\psi|^{2}\!\le\!\lambda_{2}^{\max}\sigma_{\psi}^{2}$;
combined with $\sigma_{|\psi|}^{2}\!\to\!0$ from (B2) and the
phase-variance bound from $(SG)$, both quantities go to zero
at the spectral-gap rate.\qed
\end{proof}

\noindent\emph{Empirical certificate.}
\pyown{src/derive_amplitude_gradient_lemma.py}{derive_amplitude_gradient_lemma}
confirms both $\sigma_{|\psi|}^{2}\!\to\!0$ (CI95
$[8.06\!\times\!10^{-7},\,3.69\!\times\!10^{-6}]$, below the
$10^{-3}$ structural-noise floor) and
$|\nabla\psi|^{2}\!\to\!0$ (CI95
$[-1.18\!\times\!10^{-4},\,1.29\!\times\!10^{-3}]$, containing
zero).

\begin{lemma}[K/Q recoil, KQ-recoil]\label{lem:kq_recoil}
Under $(SG)$ and the chirality-mixing closure of
P4B~\cite{Paper4B}~Eq.~KQ-full-closure (eight
System-$\mathcal{R}$ rational coefficients fixed by
$(\gamma,N_{\rm gen},d)$), the
$K/Q$ recoil contribution to $T_{00}^{\Xi}$
\begin{equation}
S_{4}(N)\;=\;\tfrac12\langle K\rangle_{\rm per}(N)
+\tfrac14-\tfrac14\langle Q\rangle_{\rm per}(N)
\label{eq:S4_definition}
\end{equation}
asymptotes to the System-$\mathcal{R}$ rational
\begin{equation}
S_{4}(N)\;\xrightarrow[N\to\infty]{}\;
\alpha_{\xi}^{2}(1-2\gamma^{2})\;=\;\frac{3969}{5000}.
\label{eq:lemma_KQ}
\end{equation}
The branch-resolved sharpening identifies
$S_{4}^{\rm mat}\!\to\!\alpha_{\xi}^{2}$ (matter-branch,
$\alpha_{\xi}^{2}$ EXACT) and
$S_{4}^{\rm vac}\!\to\!1-N_{\rm gen}/2^{d}\!=\!13/16$
(vacuum-branch, Clifford-algebra-anchored, PRECISE);
Eq.~\eqref{eq:lemma_KQ} is the chirality-flip-averaged pooled
reading.
\end{lemma}

\begin{proof}[Sketch]
\emph{(C1)} The chirality-mixing closure of
P4B~\cite{Paper4B}~Eq.~KQ-full-closure fixes
$\langle K\rangle(N)$ and $\langle Q\rangle(N)$ as
parameter-free functions of $\theta_{\rm chir}(N)$ in
$(\gamma,N_{\rm gen},d)$.
\emph{(C2)} The canonical Galerkin variation of $T_{00}^{\Xi}$
(\pyown{src/verify_galerkin_runner_A_hessian_ricci.py}{verify_galerkin_runner_A_hessian_ricci})
isolates the K/Q recoil contribution as
$S_{4}\!=\!\zeta_{3}\Omega(A_{K}\langle K\rangle\!+\!A_{Q}(1-\langle Q\rangle))$
with canonical coefficients
$\zeta_{3}\!=\!\tfrac12,\Omega\!=\!1,A_{K}\!=\!1,A_{Q}\!=\!\tfrac12$.
\emph{(C3)} The matter-localised top-5\% support
$S_{\rm src5}$ on which $\langle K\rangle_{\rm per}$ and
$\langle Q\rangle_{\rm per}$ are read inherits the
chirality-mixing structure of (C1) modulo bounded finite-$N$
corrections (P4B~\cite{Paper4B} Sec.~KQ-top5-closure).
\emph{(C4) Pooled landing.}
The unique 5-smooth System-$\mathcal{R}$ rational inside the
bootstrap-95\% CI of the Symanzik-extrapolated $S_{4}$ is
$\alpha_{\xi}^{2}(1-2\gamma^{2})$; this is precisely the
algebraic remainder of $T_{00}^{\Xi}\!\to\!\alpha_{\xi}^{2}
(1-\gamma^{2})$ after subtracting the row-variance contribution
$\alpha_{\xi}^{2}\gamma^{2}$ of Lemma~\ref{lem:row_variance_RV}.\qed
\end{proof}

\noindent\emph{Empirical certificate.}
\pyown{src/derive_kq_recoil_lemma.py}{derive_kq_recoil_lemma}
confirms $S_{4}$ Symanzik asymptote
$0.7967$ with CI95 $[0.7929,0.8005]$, containing the target
$3969/5000\!=\!0.7938$. The K/Q chirality-mixing audit
(\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_factor_field_KQ_full_closure.py}{verify_factor_field_KQ_full_closure})
returns 6 of the 8 coefficients within $1\sigma$ of their
System-$\mathcal{R}$ targets on the canonical 12-seed ladder.

\begin{theorem}[$T_{00}^{\Xi}$ continuum limit, T00]
\label{thm:t00_continuum}
Under $(SG)+A_{1}\!-\!A_{8}+$\,sync-lock and the chirality-mixing
closure of P4B~\cite{Paper4B}~Eq.~KQ-full-closure, the four-term
per-node $T_{00}^{\Xi}$ decomposition
\begin{equation*}
T_{00}^{\Xi}(N) \;=\;\tfrac12\sigma^{2}_{\Xi}(N) +
\sigma^{2}_{|\psi|}(N) + |\nabla\psi|^{2}(N) + S_{4}(N)
\end{equation*}
asymptotes by Lemmas~\ref{lem:row_variance_RV},
\ref{lem:amp_grad_AG}, \ref{lem:kq_recoil} to
\begin{equation}
T_{00}^{\Xi}(N)\;\xrightarrow[N\to\infty]{}\;
\alpha_{\xi}^{2}\gamma^{2}+0+0+
\alpha_{\xi}^{2}(1-2\gamma^{2})
\;=\;\alpha_{\xi}^{2}(1-\gamma^{2})
\;=\;\frac{8019}{10000}.
\label{eq:thm_t00_continuum}
\end{equation}
\end{theorem}

\noindent The structural-sum identity
$\alpha_{\xi}^{2}\gamma^{2}\!+\!\alpha_{\xi}^{2}(1-2\gamma^{2})\!=\!\alpha_{\xi}^{2}(1-\gamma^{2})$
is exact in $\mathbb Q$. Theorem~\ref{thm:t00_continuum}
upgrades the $T_{00}^{\Xi}$ leg of the continuum-backreaction
identity (CBI) from a single Symanzik fit of the noisy aggregate
($R^{2}\!=\!0.54$) to a chain of three lemmas plus an exact
algebraic identity. The conditionality is the same as that of
the composition theorem (M3): rigorous as a chain of lemmas
modulo $(SG)$, admissibility, and the chirality-mixing closure
of P4B.
...
\begin{remark}[Branch-resolved sharpening of $T_{00}^{\Xi}$,
$G_{00}$, $\Lambda_{t}$]
\label{rmk:cbi_branch_resolved}
The $R^{2}\!=\!0.54$ ladder dependence above reflects
\emph{branch-mixing} across the chirality flip. The
branch-resolved analysis of
Eq.~\eqref{eq:lambda_t_branch_resolved} identifies each leg
with a clean System-$\mathcal R$ rational on each branch at
sub-percent residual:
$T_{00}^{\rm vac}\!=\!\alpha_{\xi}^{2}\!+\!3\gamma^{2}\!=\!84/100$
($+0.12\%$),
$\Lambda_{t}^{\rm vac}\!=\!\alpha_{\xi}^{2}\!+\!3\gamma^{2}/2
\!=\!33/40$ ($+0.04\%$, the tightest landing in the corpus),
$G_{00}^{\rm vac}\!=\!3\gamma^{2}/2\!=\!3/200$ ($2\%$);
$T_{00}^{\rm mat}\!=\!\Lambda_{t}^{\rm mat}\!=\!\alpha_{\xi}^{2}\!=\!81/100$
($\le\!0.31\%$),
$G_{00}^{\rm mat}\!=\!\gamma^{2}/3\!=\!1/300$ ($10\%$).
\emph{Consequence for the CBI} (Theorem~\ref{thm:cbi}). The
$\kappa_{t}\!\to\!1$ statement is \emph{exact} on the matter
branch: both legs converge to $\alpha_{\xi}^{2}\!=\!81/100$
and $G_{00}\!\to\!0$ structurally, so the gravitating
stress-energy $T_{00}^{\rm grav}\!=\!T_{00}^{\Xi}\!-\!\Lambda_{t}/(8\pi G)$
vanishes identically on the matter branch. On the vacuum
branch $\kappa_{t}\!\to\!\Lambda_{t}^{\rm vac}/T_{00}^{\rm vac}
\!=\!(33/40)/(84/100)\!=\!55/56\!\approx\!0.9821$ and
$G_{00}\!\to\!3\gamma^{2}/2\!\ne\!0$ --- the CBI is approximate,
not exact, on this branch. The pooled $R^{2}\!=\!0.54$
extrapolation of Lemma~\ref{lem:t00-asymptote} reflects
branch-averaging; per-branch the asymptotes are not numerically
uncertain but algebraically pinned to System-$\mathcal R$
rationals. The cosmological $z\!=\!0$ universe sits on the
matter branch, so the exact CBI closure is the operative one
for late-time observables.
\end{remark}
...
\begin{theorem}[Continuum Backreaction Identity, CBI --- conditional on the empirical asymptotes]
\label{thm:cbi}
\emph{Conditional statement.} Assume \emph{(a)} the
empirical-asymptote Lemma~\ref{lem:t00-asymptote} --- the
Symanzik~$2$ extrapolation of $T_{00}^{\Xi}(N)$ converges to an
$\mathcal R$-rational continuum value $\tau_{\infty}$ (nominally
$\alpha_{\xi}^{2}$, or its $\gamma^{2}$-shift
$\alpha_{\xi}^{2}(1\!-\!\gamma^{2})$ on the wider snapshot
ladder; see Lemma~\ref{lem:t00-asymptote} and
Remark~\ref{rmk:t00_summand_decomposition}) --- and \emph{(b)}
the auxiliary asymptote $G_{00,\mathrm{med}}(N)\!\to\!0$ (an
$N^{-1.32}$ decay verified at $R^{2}=0.99$). These are numerical
extrapolations ($R^{2}=0.54$ for the $T_{00}^{\Xi}$ leg),
\emph{not} closed-form analytic limits; see the per-Lemma
disclosures. Lemma~\ref{lem:lambda-t-asymptote} is then
\emph{not} an independent assumption but a \emph{corollary} of
(a) and (b): under the per-regime optimal definition
$\Lambda_{t}=T_{00}^{\Xi}-G_{00}$ the $\Lambda_{t}$ leg inherits
the $T_{00}^{\Xi}$ limit whatever its value. Then, under the
$\mathcal R$ canonical coefficient choice and the per-regime
optimal definition of $\Lambda_{t}$,
the cosmological-tensor time-time component asymptotically
equals the relational stress-energy time-time component in the
continuum limit:
\begin{equation}
\lim_{N\to\infty} \Lambda_{t}(N) \;=\;
\lim_{N\to\infty} T_{00}^{\Xi}(N) \;=\; \tau_{\infty},
\qquad\Longrightarrow\qquad
\kappa_{t}^{\infty} \;\equiv\; \lim_{N\to\infty}
\frac{\Lambda_{t}(N)}{T_{00}^{\Xi}(N)} \;=\; 1,
\label{eq:cbi}
\end{equation}
the ratio conclusion holding for either admissible value of the
common limit $\tau_{\infty}$.
...
and \ref{lem:lambda-t-asymptote}. The load-bearing fact is that
the two legs share a \emph{common} continuum value: under the
per-regime optimal definition $\Lambda_{t}=T_{00}^{\Xi}-G_{00}$
with $G_{00}\to 0$, the $\Lambda_{t}$ leg inherits the
$T_{00}^{\Xi}$ leg's limit whatever that limit is, so the ratio
$\kappa_{t}=\Lambda_{t}/T_{00}^{\Xi}$ asymptotes to one. In
particular, the ratio conclusion is \emph{invariant} under the
$\alpha_{\xi}^{2}$ versus $\alpha_{\xi}^{2}(1\!-\!\gamma^{2})$
ladder ambiguity of Lemma~\ref{lem:t00-asymptote}: a shared
$\gamma^{2}$-shift cancels in the ratio. The corollary on
...
\noindent\emph{Logical status.} The CBI is a \emph{conditional
implication} whose antecedent reduces to \emph{one} empirical
asymptote --- the Symanzik convergence of $T_{00}^{\Xi}(N)$,
$R^{2}=0.54$ --- plus \emph{one} strongly-supported auxiliary,
$G_{00,\mathrm{med}}\!\to\!0$ at $R^{2}=0.99$;
Lemma~\ref{lem:lambda-t-asymptote} is a corollary of these, not
an independent input. The deduction
``both legs $\to$ a common value $\Rightarrow$ ratio $\to 1$''
is exact algebra; what is empirical, and therefore the
load-bearing assumption, is that the $T_{00}^{\Xi}$ leg
\emph{does} converge --- the coincidence of the two limits is
then forced, not assumed. This is the
robust part: $\Lambda_{t}=T_{00}^{\Xi}-G_{00}$ with
$G_{00}\to 0$ forces the two limits to coincide whatever the
common value, so the ratio conclusion does not depend on
pinning that value to $\alpha_{\xi}^{2}$ rather than its
$\gamma^{2}$-shift. The within-$\mathcal R$ algebra of the
implication is exact; the conditional input is the convergence
of the two legs, currently supported by Symanzik extrapolations
with a sub-percent relative offset on the $T_{00}^{\Xi}$ leg. A
closed-form analytic proof of either asymptote would
additionally pin the common value and promote the CBI to an
...
\smallskip\noindent\emph{Hypothesis-level discharge table.}
The composition is made precise by mapping each classical
hypothesis to the discrete condition(s) that discharge it, and
labelling the status of each implication:

{\small
\begin{center}
\begin{tabular}{@{}p{0.31\linewidth}p{0.20\linewidth}p{0.40\linewidth}@{}}
\toprule
Classical hypothesis required & Discharged by & Status of the implication \\
\midrule
mm-GH precompactness (Sturm~\cite{Sturm2006} $\mathbb{D}$-topology):
uniform doubling $+$ uniform tightness
& (A1)$+$(A2)$+$(A8)
& \textsc{rigorous}. (A8) is the two-sided
$c\rho^{d_*}\!\le\!\mu(B)\!\le\!C\rho^{d_*}$ doubling/non-collapse
bound verbatim; (A1)$+$(A2) supply the degree bound and
common-compact tightness Gromov compactness needs. \\
\addlinespace
Limit is a compact length space:
local chart consistency on overlaps
& (A4)$+$(A7)
& \textsc{rigorous} for the length-space structure; the
\emph{choice} of $\Gamma_{r_N}$ and the $r\!\mapsto\!br$
robustness functional as the consistency measures is
framework-defined, not classical. \\
\addlinespace
Limit is $d_*$-rectifiable and Ahlfors $d_*$-regular:
non-collapse $+$ fixed effective dimension
& (A8)$+$(A6)
& \textsc{rigorous}. (A8) is Ahlfors $d_*$-regularity; (A6)
pins the dimension parameter $d_*$. \\
\addlinespace
$g_{\rm eff}$ exists $\mu_\infty$-a.e.\ as the $L^\infty$-limit
of patch metrics: patch metrics Cauchy under refinement
& (A3)$+$(A4)$+$(A7)
& \textsc{rigorous} \emph{given} that the framework distortion
functionals $E_{\rm nstress},\Gamma_{r_N}$ vanish in the limit
(which is what (A3),(A4) assert). \\
\addlinespace
Curvature-dimension control for the limit: a uniform lower
bound $CD(K_{\mathrm{CD}},N)$ (Sturm~\cite{Sturm2006}) / the
Lott--Villani measure-contraction
property~\cite{LottVillani2009}, where $K_{\mathrm{CD}}$ is the
curvature-dimension lower-bound parameter of Sturm's condition
(unrelated to the corpus chirality-mixing operator $K$ of the
$K/Q$ closure family)
& (A5)$+$(A6)
& \textsc{open at theorem level; empirically witnessed}.
(A5) as stated gives only $\Delta_{\rm curv}\!\to\!0$ --- the
curvature \emph{stabilises} --- which is \emph{not} a uniform
lower bound $K_{\mathrm{CD}}$; (A6) fixes $N\!=\!d_*$ but not
$K_{\mathrm{CD}}$. Deriving $CD(K_{\mathrm{CD}},N)$ or the
measure-contraction property from (A5)$+$(A6) is the one
load-bearing implication this sketch does \emph{not} close as a
theorem. The \emph{sign} information it requires is, however,
supplied empirically by two independent in-repo audits on the
canonical ladder (see the witness list below): the signed
Hessian-discrepancy Ricci minimum eigenvalue extrapolates to a
\emph{finite} limit with a non-negative point estimate (its
bootstrap CI straddling zero, so consistent with $CD(0,N)$),
and the intrinsic Bakry--\'Emery $\Gamma_2$ curvature of the
carrier Dirichlet form extrapolates to a finite \emph{strictly
positive} limit --- together a uniform $CD(K_{\mathrm{CD}},N)$
bound with $K_{\mathrm{CD}}\!\geq\!0$ in the robust-percentile
sense. \\
\bottomrule
\end{tabular}
\end{center}}

\noindent Four of the five classical hypotheses are discharged
rigorously (modulo the framework-defined choice of distortion
functionals, which is a definitional rather than a logical
gap). The fifth row --- the curvature-dimension lower bound --- is
the single implication open \emph{at theorem level}: this is
precisely \emph{why} the result is stated as ``[Sketch]'' and
used downstream only in the conditionally-rigorous sense of
Corollary~\ref{cor:cbi_admissibility}. Promotion to a full
``[Proof]'' requires closing the
(A5)$+$(A6)$\,\Rightarrow\,CD(K_{\mathrm{CD}},N)$
row --- e.g.\ by strengthening (A5) from curvature-\emph{stability}
to a uniform curvature \emph{lower bound}, or by an independent
discrete-to-continuum measure-contraction argument. Both of
these routes are now executed \emph{empirically} on the
canonical ladder, each independently witnessing a finite
non-negative $K_{\mathrm{CD}}$
(Paragraph~\ref{par:cd_empirical_witnesses} below): the analytic
discharge of the row is open, but the bound it asserts is no
longer unsupported.
\end{proof}
...
\begin{itemize}
\item \pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_continuum_bianchi.py}{verify_continuum_bianchi}, output
...
\item \pyown{src/stage6f_fine_percentile_audit.py}{stage6f_fine_percentile_audit}, output
...
(A4)+(A8).
\item \pyown{src/verify_lambda_extrapolation_discipline_audit.py}{verify_lambda_extrapolation_discipline_audit},
...
\item \pyown{src/verify_clp_c_gamma_convergence_detailed.py}{verify_clp_c_gamma_convergence_detailed},
...
\paragraph{Empirical witnesses of the curvature-dimension lower
bound.}
\label{par:cd_empirical_witnesses}
The single open row of the discharge table,
(A5)$+$(A6)$\,\Rightarrow\,CD(K_{\mathrm{CD}},N)$, asks for the
\emph{sign} of the limiting curvature, not merely its stability.
Two further bundled reproducers supply that sign by the two
routes named in the proof above, each operating on the same
canonical ladder $N\!\in\![50,512]$ with seed-bootstrap $95\%$
confidence intervals:
\begin{itemize}
\item \emph{Strengthened-(A5) route ---} the signed
Hessian-discrepancy Ricci tensor.
\pyown{src/verify_signed_ricci_lower_bound.py}{verify_signed_ricci_lower_bound}, output
\path|outputs/signed_ricci_lower_bound_audit.json|: the per-node
minimum eigenvalue of the Bochner-convention Ricci tensor
$R^{\rm Hess}_{ij}$ of \S\ref{sec:cbi} (Runner~A) is extracted,
and the regime curvature lower bound
$K_N=\inf_a\lambda_{\min}(R^{\rm Hess}(a))$ Symanzik-extrapolated.
The robust percentile $K_{p1}$ extrapolates to a finite limit
whose point estimate is non-negative ($\hat K_{p1}^{\infty}
\!\approx\!+7.5\times10^{-4}$), though with a wide bootstrap CI
$[-0.008,+0.008]$ that straddles zero and a weak fit
($R^{2}\!\approx\!0.51$); the more robust qualitative signal is
the hyperbolic-node fraction $f_{\rm neg}\!\to\!0$ --- zero
nodes of negative minimum Ricci eigenvalue for $N\!\geq\!128$.
Read together: (A5) is witnessed not merely as
$\Delta_{\rm curv}\!\to\!0$ but as $\lambda_{\min}(R^{\rm Hess})$
\emph{bounded below}, with the limiting bound consistent with
(and not distinguishable from) zero at the present resolution.
\item \emph{Measure-contraction route ---} the intrinsic
Bakry--\'Emery $\Gamma_2$ curvature of the carrier random-walk
Dirichlet form. \pyown{src/verify_carrier_bakry_emery_cd.py}{verify_carrier_bakry_emery_cd},
output \path|outputs/carrier_bakry_emery_cd_audit.json|: the
pointwise Bakry--\'Emery curvature
$K_x=\min\{\lambda:\Gamma_2(f,f)(x)\!\geq\!\lambda\,\Gamma(f,f)(x)\}$
is computed exactly as the smallest generalised eigenvalue of
the local $\Gamma_2/\Gamma$ form pair (no surrogate; a
disclosed vertex/seed subsample keeps the per-vertex
generalised eigenproblem tractable on the full ladder), and the
regime curvature Symanzik-extrapolated. The robust percentile
$K_{p5}$ extrapolates to a finite \emph{strictly positive}
limit ($\hat K_{p5}^{\infty}\!\approx\!0.49$, $R^{2}\!\approx\!0.33$),
positive uniformly at \emph{every} regime on the ladder: the
carrier sequence numerically witnesses a uniform
$CD(K_{\mathrm{CD}},\infty)$ bound with $K_{\mathrm{CD}}>0$ in
the robust-percentile sense, which with (A6) pinning
$N\!=\!d_*$ is the $CD(K_{\mathrm{CD}},N)$ bound the open row
asserts.
\end{itemize}
Both routes are discrete-graph instances of the now-established
link between a network/graph curvature and the continuum Ricci
curvature of the underlying space (van der
Hoorn~et~al.~\cite{vanderHoorn2021}, who prove Ollivier-Ricci
convergence in random geometric graphs). By the stability of the
$CD/RCD$ condition under measure-Gromov--Hausdorff convergence
(Sturm~\cite{Sturm2006}), a uniform finite lower bound along the
sequence is inherited by the mm-GH limit. The open row therefore
remains open \emph{as an analytic discharge} --- a closed-form
derivation of $K_{\mathrm{CD}}$ from (A5)$+$(A6) is still
required for a full ``[Proof]'' --- but the curvature-dimension
bound it asserts is now witnessed empirically by two independent
constructions, the extrinsic Hessian-Ricci sign and the
intrinsic Dirichlet-form $\Gamma_2$ curvature, in mutual
agreement (Figure~\ref{fig:cd_curvature_witnesses}).

\emph{Physical-reading proposal for the open analytic
discharge.}
The empirical witnessing $K_{\rm CD}\!\geq\!0$ is plausibly a
\emph{corollary of the $(SG)$-axiom
$\lambda_{\infty}\!=\!(d{-}1)/(2d)\!=\!3/8\!>\!0$} via the
Ollivier--Lin--Lu--Yau coarse-Ricci-curvature/Laplacian-
spectral-gap correspondence on configuration-model graphs with
positive clustering~\cite{Ollivier2009,LinLuYau2011}. Heuristic
chain: (i) positive Laplacian spectral gap
$\lambda_{\infty}\!>\!0$ implies, via the Lichnerowicz-type
inequality on discrete metric measure spaces, a strictly
positive lower coarse Ricci-curvature bound
$\kappa_{\rm Ollivier}\!\geq\!2\lambda_{\infty}/N$ in the
locally-bounded-degree regime; (ii) $\kappa_{\rm Ollivier}\!>\!0$
in turn implies $K_{\rm CD}\!\geq\!0$ in the Sturm--Lott--Villani
sense (Bakry--\'Emery $\Gamma_{2}$-calculus closes with
non-negative $K$ on positively-clustered configuration-model
graphs); (iii) the Newman-clustered structure of the carrier
(certified in Proposition~\ref{prop:gap_statistical_fingerprint}
via the $(SG)$-spectral-fingerprint coincidence at
$\lambda_{\infty}\!=\!7/24$) realises step (i) for the specific
ensemble that the carrier limit is taken in. Under this
proposal the analytic discharge of CD$(K_{\rm CD}\!\geq\!0, N)$
reduces to the analytic discharge of the $(SG)$-axiom itself
(Lemma~B Step-4), and the two empirical witnesses
(signed Hessian-Ricci, Bakry--\'Emery $\Gamma_{2}$) are the
two-side check that the chain (i)$\to$(ii) closes
numerically. This proposal is \emph{not} a theorem at the
present level of rigor; it is a physical-reading conjecture
that simultaneously motivates the empirical positivity and
identifies the single open lemma whose discharge would close
CD$(K_{\rm CD}\!\geq\!0)$ as a corollary.

\paragraph{Quantitative cross-projection: three Bakry--\'Emery
percentile statistics close to $\lambda_{\infty}\!\times\!(d,
N_{\rm gen})$-rationals.}
\label{par:cd_cross_projection}
The qualitative positivity above is sharpened to a
\emph{quantitative} cross-projection between the
$(SG)$-axiom and the Bakry--\'Emery $\Gamma_{2}$ audit. The
empirical $K_{\rm BE}$ asymptotes for three distinct
percentile statistics on the canonical ladder
$N\!\in\![50, 512]$ (24 seeds) are recorded in
\path|outputs/carrier_bakry_emery_cd_audit.json|; the
verifier
\pyown{src/verify_CD_K_N_cross_projection.py}{verify_CD_K_N_cross_projection}
tests their identification with
$\lambda_{\infty}\!\cdot\!\text{rational}(d, N_{\rm gen})$:
\begin{align}
\label{eq:K_BE_p1_cross_projection}
K_{\rm BE}^{p_{1}}     &\;=\;
   \lambda_{\infty}\,\frac{d{+}1}{d}
   \;=\;\frac{3}{8}\!\cdot\!\frac{5}{4}\;=\;\frac{15}{32}\!=\!0.46875
   \quad (\text{empirical }0.4697,\;
   \Delta\!=\!-0.19\%) \\
\label{eq:K_BE_p5_cross_projection}
K_{\rm BE}^{p_{5}}     &\;=\;
   \lambda_{\infty}\,\frac{N_{\rm gen}(d{+}N_{\rm gen})}{2^{d}}
   \;=\;\frac{3}{8}\!\cdot\!\frac{21}{16}\;=\;\frac{63}{128}\!=\!0.49219
   \quad (\text{empirical }0.4924,\;
   \Delta\!=\!-0.04\%) \\
\label{eq:K_BE_mean_cross_projection}
K_{\rm BE}^{\rm mean}  &\;=\;
   \lambda_{\infty}\,\frac{4d{+}1}{2d{+}N_{\rm gen}}
   \;=\;\frac{3}{8}\!\cdot\!\frac{17}{11}\;=\;\frac{51}{88}\!=\!0.57955
   \quad (\text{empirical }0.5790,\;
   \Delta\!=\!+0.10\%)
\end{align}
All three predictions sit inside the bootstrap $95\%$ CIs:
$[0.4525, 0.4857]$, $[0.4787, 0.5064]$, $[0.5659, 0.5915]$
respectively. Three independent percentile statistics match
three independent framework rationals at PRECISE tier (residuals
$0.04$--$0.19\%$) --- the Bakry--\'Emery curvature distribution
is structurally fixed by the $(SG)$-axiom $\lambda_{\infty}$ plus
$(d, N_{\rm gen})$ algebra. The three rationals admit clean
physical readings: the lower-tail
$(d{+}1)/d$ is the sync-cone-axis-to-spatial-dimension ratio
(worst-case curvature retains a sync-cone safety margin); the
robust-percentile $N_{\rm gen}(d{+}N_{\rm gen})/2^{d}$ is the
ratio of total matter-mode count to the Clifford algebra
dimension $\mathrm{Cl}(d)\!=\!2^{d}\!=\!16$ (spectrum-times-
matter-fraction reading); the distributional mean
$(4d{+}1)/(2d{+}N_{\rm gen})$ is the chirality-spin-extended-
axis count divided by the EW-axis count $P_{29}$-first-piece
$2d{+}N_{\rm gen}\!=\!11$. The cross-projection between the
$(SG)$-axiom and Bakry--\'Emery $\Gamma_{2}$ calculus is
therefore three-fold independent: three empirical statistics
map to three framework rationals, all derived from the
same master input $\lambda_{\infty}\!=\!3/8$.
The physical mechanism for the curvature-dimension lower bound
is then the cross-projection of the $(SG)$-axiom spectral gap
onto the Bakry--\'Emery curvature distribution via
$(d, N_{\rm gen})$-algebra, analogous to the Friedmann
cross-projection that delivers the local Hubble constant from
independent
$\rho_{\Lambda}\!\times\!\Omega_{\Lambda}$ inputs in the
companion paper~\cite{Paper4B}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.99\textwidth]%
{figures/fig18_cd_curvature_witnesses.pdf}
\caption{Empirical witnesses of the curvature-dimension lower
bound $CD(K_{\mathrm{CD}},N)$ on the canonical-physics ladder
$N\!\in\![50,512]$, the two routes named in the proof of
Theorem~\ref{thm:hard_xi_continuum}.
\textbf{(a)} Strengthened-(A5) route
(\texttt{src/verify\_signed\_ricci\_lower\_bound.py}): the robust
percentile lower bounds $K_{p1},K_{p5}$ of the signed
Hessian-discrepancy Ricci minimum eigenvalue extrapolate to a
finite limit with a non-negative point estimate (bootstrap CI
straddling zero, hence consistent with $CD(0,N)$), and the
hyperbolic-node fraction $f_{\rm neg}$ (red, right axis)
vanishes for $N\!\geq\!128$.
\textbf{(b)} Measure-contraction route
(\texttt{src/verify\_carrier\_bakry\_emery\_cd.py}): the intrinsic
Bakry--\'Emery $\Gamma_{2}$ curvature percentiles
$K_{\inf},K_{p5},K_{\rm mean}$ of the carrier random-walk
Dirichlet form are uniformly \emph{positive} across the ladder;
the robust $K_{p5}$ asymptote (green star) is bounded away from
zero. Both routes independently support a uniform
$CD(K_{\mathrm{CD}},N)$ bound with $K_{\mathrm{CD}}\!\geq\!0$.}
\label{fig:cd_curvature_witnesses}
\end{figure}
...
\pyown{src/verify_clp_c_gamma_convergence_detailed.py}{verify_clp_c_gamma_convergence_detailed}, bundled
output
...
\emph{Non-independence disclosure.} The C5 proxy is by
construction the arithmetic mean
$\tfrac12(\mathrm{C1}+\mathrm{C2})$, i.e.\ it is \emph{not}
an independent fifth measurement of the
$\delta S_N\!\to\!\delta S_\infty$ condition but a derived
quantity. The five-entry mean $\gamma$-score therefore weights
the C1/C2 information twice. With C5 excluded, the mean of the
four genuinely-independent proxies is
$\tfrac14(0.732+1.000+0.832+0.816)=0.845$ --- statistically
indistinguishable from the reported $0.849$, so the
\texttt{GAMMA\_CONVERGED} verdict is unchanged either way; but
the aggregate should be read as a four-proxy certificate with
a redundant fifth row, not as five independent conditions.
\end{theorem}
\begin{proof}[Reproduction.]
Direct numerical reproduction: run
\pyown{src/verify_clp_c_gamma_convergence_detailed.py}{verify_clp_c_gamma_convergence_detailed}; the
...
\paragraph{Numerical-to-analytical proxy map (transparency).}
The C1--C5 numerical proxies above are approximations of the
five standard analytical conditions of Dal Maso
$\Gamma$-convergence~\cite{DalMaso1993}; an unconditional
proof would replace each numerical proxy by a rigorous
analytical argument. The mapping is:
\begin{itemize}
\item[\textbf{C1}] \emph{Liminf inequality}: for every sequence
$f_{N}\!\to\!f$ in the mm-GH limit, $\liminf_{N}\!S_{N}(f_{N})\!\ge\!S_{\infty}(f)$.
Numerical proxy: total closure loss
$L(N)\!=\!\mathrm{mean}(1\!-\!\mathrm{gap}_{i})$ on six payload
keys, Symanzik-extrapolated to $L_{\infty}\!\in\![0.370,0.398]$
($95\%$~CI); proxy score $0.732$.
\item[\textbf{C2}] \emph{Recovery sequence}: for every $f$
there exists a sequence $f_{N}\!\to\!f$ with
$\limsup_{N}\!S_{N}(f_{N})\!\le\!S_{\infty}(f)$.
Numerical proxy: $\min$ of the per-payload canonicalisation
score, hitting $1.0$ identically on the bundled ladder;
proxy score $1.000$.
\item[\textbf{C3}] \emph{Equi-coercivity}: for every $M$,
the sub-level set $\{f\!:\!S_{N}(f)\!\le\!M\}$ is contained in
a fixed compact set independent of $N$.
Numerical proxy: $1\!-\!\mathrm{range}(\text{transport})\!=\!0.832$;
proxy score $0.832$.
\item[\textbf{C4}] \emph{Minimiser convergence}: minimisers
$f_{N}^{*}$ of $S_{N}$ converge to a minimiser of $S_{\infty}$.
Numerical proxy: fixpoint-proximity asymptote
$\mathrm{score}\!=\!0.816$ with bootstrap-$95\%$~CI
$[0.780,0.865]$.
\item[\textbf{C5}] \emph{Euler--Lagrange link}: weak-$*$
convergence of first variations,
$\delta S_{N}\!\to\!\delta S_{\infty}$.
Numerical proxy: $\tfrac{1}{2}(\mathrm{C1}\!+\!\mathrm{C2})\!=\!0.866$
(C1 supplies the lower envelope, C2 the upper envelope; their
average is the standard weak-$*$ link).
\end{itemize}
An analytical replacement of any one $C_{i}$ tightens the
composition theorem along its corresponding axis; replacing
all five completes the analytical Lemma~C alongside the
analytical Lemma~B (the target $(\mathrm{SG})$-Lemma of
Section~\ref{sec:gap} and the corresponding Phase-$2$
discussion). The current
$\gamma$-score $0.849$ certifies the conditional
composition theorem at $95\%$ bootstrap confidence on the
$9$-payload ladder, sufficient for the conditional Phase-$1$
result of P6~\cite{Paper6} but not for an unconditional
continuum theorem.

\paragraph{Lemma~C is a multi-axis target: CLP-A + CLP-B
+ CLP-C.}
The Dal Maso $\Gamma$-convergence sub-conditions C$1$--C$5$
above form the CLP-C axis of the closure-limit-proof
framework. The full Lemma~C target also includes the
\emph{operator-convergence axis} CLP-B and the
\emph{existence axis} CLP-A; all three are reported in the
bundled audit
\path|outputs/clp_full_report.json|:
\begin{itemize}
\item \textbf{CLP-A} (continuum-limit existence):
$0.813$ \textbf{CLOSED} on the $9$-payload ladder
(sub-scores T$1\!=\!1.0$, T$2\!=\!0.90$, T$3\!=\!0.45$,
T$4\!=\!1.0$).
\item \textbf{CLP-B/B$4$} (operator convergence):
$0.598$ overall with four sub-components
absorption $\!=\!0.514$ (bottleneck), locality $\!=\!0.542$,
density $\!=\!0.652$, spectral $\!=\!0.682$
(\path|outputs/clp_full_families_extended_audit.json|,
\texttt{clp\_b\_b4} key).
\item \textbf{CLP-C} (the $\Gamma$-convergence Dal Maso
audit reported above): $0.849$ \textbf{GAMMA\_CONVERGED}
with C$1\!=\!0.73$, C$2\!=\!1.00$, C$3\!=\!0.83$,
C$4\!=\!0.82$, C$5\!=\!0.87$.
\item \textbf{CLP-D overall}: $0.738$ \textbf{CLP\_PROVEN}
(weights $A\!:\!0.3, B\!:\!0.4, C\!:\!0.3$;
\path|outputs/clp_full_report.json|, \texttt{CLP-D} key).
\end{itemize}
The Lemma~C analytical-replacement programme therefore
proceeds along three axes simultaneously: tighten the
worst-scoring C$_{i}$ (currently C$1$=liminf at $0.73$),
tighten the worst-scoring B sub-component (currently the
absorption bottleneck at $0.514$), and (where data
permits) tighten the worst-scoring CLP-A T-condition
(currently T$3$ at $0.45$). The conditional master
closure theorem of P6~\cite{Paper6} holds at
CLP\_PROVEN level (CLP-D $\!=\!0.738$) across all three
axes; the unconditional theorem requires analytical
replacement of the weakest-scoring sub-component on each
axis.
...
\pyown{src/verify_admissibility_counterexample_and_spectral_gap.py}{verify_admissibility_counterexample_and_spectral_gap}.
...
target ``$(\mathrm{SG})$-Lemma'']
\label{def:uniform_spectral_gap}
\textbf{Disambiguation.} The ``$(\mathrm{SG})$-Lemma''
discussed below is the single target theorem about the
carrier-action uniform spectral gap. It is distinct from
the topology-labelled loop-class library entries
``Lemma~$1$,~Lemma~$2$,~\ldots,~Lemma~$10$'' of
P3~\cite{Paper3}, which are deterministic single-valued
read-offs for individual cross-sector observables and form
a finite library, not a target theorem.
...
\pyown{src/verify_lemma_B_uniform_poincare.py}{verify_lemma_B_uniform_poincare}
(output
...
$3/8\!=\!(d-1)/(2d)\!=\!0.375$ within $1.0\%$ relative
residual. The conjecture $\lambda_{\infty}\!=\!3/8$ is
registered for future falsification on an extended ladder
$N\!\ge\!1024$, not claimed as a closure.

\textbf{Branch scope and branch-resolved closure.} The
Phase-2 ladder $N\!\in\![50,512]$ \emph{spans} the matter-
vacuum chirality flip, which occurs at the dynamical
crossing $\theta_{\mathrm{chir}}(N^{\!*})\!=\!\pi/4$
between $N\!=\!100$ ($\theta_{\mathrm{chir}}\!=\!37.1^{\circ}$)
and $N\!=\!128$
($\theta_{\mathrm{chir}}\!=\!46.1^{\circ}$),
i.e.\ $N^{\!*}\!\approx\!110$--$120$ (per-regime causal-wave
readout, \path|outputs/causal_wave_per_N_readout.json|;
not to be confused with the chirality-inversion endpoint
$N_{\mathrm{inv}}\!\approx\!591$--$600$ where
$\theta_{\mathrm{chir}}\!\to\!\arctan(N_{\mathrm{gen}})\!\approx\!71.6^{\circ}$).
Branch-separated Symanzik-$1$ fits identify two distinct
closed-form asymptotes:
\begin{equation}
\boxed{\;
\lambda_{\infty}^{\rm vac}\!=\!\frac{d-1}{2d}\!=\!\frac{3}{8},
\qquad
\lambda_{\infty}^{\rm mat}\!=\!\frac{d-1}{2d}+2\gamma^{2}\!=\!\frac{79}{200}
\;}
\label{eq:sg_branch_resolved}
\end{equation}
empirically:
\begin{equation*}
\begin{array}{lll}
\text{Vacuum (}N\!\le\!100,\,5\text{ pts}\text{):} & \lambda_{\infty}^{\rm vac}\!=\!0.3732, & \Delta\!=\!-0.48\%\text{ vs }3/8 \\
\text{Matter (}N\!\ge\!256,\,3\text{ pts}\text{):} & \lambda_{\infty}^{\rm mat}\!=\!0.3957, & \Delta\!=\!-0.18\%\text{ vs }79/200 \\
\text{Pooled (all }10\text{ pts):} & \lambda_{\infty}\!=\!0.3789, & \Delta\!=\!+1.04\%\text{ vs pooled } 3/8 \\
\end{array}
\end{equation*}
The matter-branch identification $79/200\!=\!3/8\!+\!2\gamma^{2}$
is the unique candidate among $\{3/8, 2/5, 79/200, 41/100\}$
that yields a constant Symanzik-$1$ coefficient $a$
(implied-$a$ spread $0.216$ across $N\!=\!256,300,512$,
vs $5.30$ for $3/8$, $1.10$ for $2/5$, $3.66$ for $41/100$);
the bootstrap-CI$_{95\%}$ $[0.394, 0.407]$ contains
$79/200$ and rejects both $3/8$ and $41/100$. The audit
is \pyown{src/verify_lemma_B_matter_branch_universality.py}{verify_lemma_B_matter_branch_universality}
(output
\path|outputs/verify_lemma_B_matter_branch_universality.json|).
The matter-branch shift from the vacuum asymptote equals
$d\gamma^{2}/2\!=\!2\gamma^{2}\!=\!1/50$ exactly --- the
natural leading sub-leading $\gamma^{2}$-correction of the
System-$\mathcal{R}$ algebra. The pooled $1.04\%$ residual
against pure $3/8$ now decomposes into vacuum-pulldown and
matter-pullup contributions consistent with
\eqref{eq:sg_branch_resolved}. A future extended ladder
$N\!\in\![512, 1024]$ that finds $\lambda_{\infty}^{\rm mat}$
outside $[0.385, 0.405]$ falsifies the matter-branch
identification.
\end{proposition}
...
\begin{remark}[Kesten--McKay baseline at the carrier mean degree]
\label{rmk:km_baseline}
A complementary structural observation, anchored in random-matrix
theory of sparse random regular graphs (Bauerschmidt--Huang--Yau,
local Kesten--McKay law for random regular graphs at fixed
degree~\cite{BauerschmidtHuangYau2019}), gives the
tree-like baseline for the normalised-Laplacian spectral gap of a
random $d$-regular graph as
\begin{equation}
\lambda_{2}^{\mathrm{KM}}(d) \;=\; 1 - \frac{2\sqrt{d-1}}{d}.
\label{eq:km_baseline}
\end{equation}
Evaluating at the carrier's empirical mean skeleton degree
$d_{\mathrm{eff}}^{\mathrm{skel}} \!=\! d \cdot N_{\rm gen} \!=\!12$
(itself a structural identification, P4 par. statistical-fingerprint)
gives the irrational
$\lambda_{2}^{\mathrm{KM}}(12) = 1 - 2\sqrt{11}/12 = 0.44723$.
The carrier's vacuum-branch asymptote $\lambda_{\infty}^{\rm vac}\!=\!3/8\!=\!0.37500$
sits \emph{below} this baseline by $0.07223$, suggestively close to
the universal-leakage constant $\gamma^{2}(d{+}N_{\rm gen})\!=\!7/100\!=\!0.07000$
that anchors the $n_{s}$, $\Sigma m_{\nu}$, $D/H_{\rm dressed}$,
and Tolman-extended closure family.
Numerically,
\begin{equation}
\lambda_{2}^{\mathrm{KM}}(12) - \gamma^{2}(d{+}N_{\rm gen})
\;=\;0.37723
\;\approx\; \tfrac{3}{8}
\qquad(\text{relative residual }+0.59\%).
\label{eq:km_minus_leakage}
\end{equation}
This decomposition is \emph{not} an algebraic identity in
$\mathbb Q$ (the right-hand side contains $\sqrt{11}/12$,
irrational), so the numerical match at $0.6\%$ is a coincidence at
the empirical precision rather than a structural derivation.
It is registered here as a falsification handle: if the carrier
spectral gap were genuinely a Kesten--McKay baseline minus a
universal-leakage triangle/clustering correction, one would expect
a graph-class-dependent corrected form whose irrational part
cancels against the carrier's specific clustering structure.
The bundled audit script
\pyown{src/derive_kesten_mckay_baseline_observation.py}{derive_kesten_mckay_baseline_observation}
checks the numerical match and registers the $0.6\%$ residual; a
closed-form derivation that yields the exact rational $3/8$ from
Kesten--McKay plus a structured triangle correction is a concrete
analytical target on the Pham--Peron--Metz cavity
machinery~\cite{PhamPeronMetz2024} for sparse networks with
clustering.
\end{remark}

\begin{remark}[Per-regime algebraic chain $3/8 = (7/24)\cdot(9/7)$
and its end-to-end empirical match]
\label{rmk:phase3_algebraic_chain}
The closure $\lambda_{\infty}^{\rm vac}\!=\!3/8$ admits the
$\mathbb Q$-identity
\begin{equation}
\frac{3}{8} \;=\; \underbrace{\frac{7}{24}}_{\lambda_{\infty}^{\rm skel}}
\;\cdot\;\underbrace{\frac{9}{7}}_{\text{weight-lift factor}},
\qquad \frac{9}{7} = \frac{d \,N_{\rm gen} \,(d-1)}{2 d (d + N_{\rm gen})}\;\Big/\;\frac{7}{24}.
\label{eq:phase3_chain}
\end{equation}
Per-regime Symanzik extrapolation on the canonical ladder
$N\!\in\!\{50,64,72,84,100,128,200,256,300,512\}$, broken into the
three factors (skeleton $\lambda_{2}$ at threshold
$\tau\!=\!0.10$; weighted $\lambda_{2}$ on the unthresholded carrier;
weight-lift ratio of the two), gives
$\widehat\lambda^{\rm skel} = 0.314$ (target $7/24\!=\!0.292$,
$+7.6\%$), $\widehat\lambda^{\rm w} = 0.396$ (target
$3/8\!=\!0.375$, $+5.5\%$), $\widehat{(\text{lift})} = 1.249$
(target $9/7\!=\!1.286$, $-2.8\%$). The chained product
$\widehat\lambda^{\rm skel} \cdot \widehat{(\text{lift})} = 0.392$
matches the directly-extracted $\widehat\lambda^{\rm w}\!=\!0.396$
to $0.88\%$ --- tighter than any single-factor residual,
so the three factors are structurally correlated rather than
algebraically independent. Conditional on the cross-sector
independence statement that $3/8$ lives in the
dimensional-graph algebra $\mathbb Q(d)$ rather than in the
System-$R$ coefficient algebra (par.~\ref{par:cross_closure_identities}),
the factor $9/7$ contains $N_{\rm gen}$ explicitly and cancels in the
product; the chain is therefore a \emph{post-hoc} factorisation
exact in $\mathbb Q$, not an independent two-step mechanism.
Audit script:
\pyown{src/derive_skeleton_weighted_lift_chain.py}{derive_skeleton_weighted_lift_chain}.
\end{remark}

\begin{remark}[Eigenvector top-spectrum is near-degenerate, not
Cartesian-product]
\label{rmk:phase4a_modes}
A natural two-factor reading of the algebraic chain
\eqref{eq:phase3_chain} would predict that the skeleton-Laplacian
spectrum factors as a Cartesian product of a $d$-mode and an
$N_{\rm gen}$-mode, giving the analytic prediction
$\lambda_{3}/\lambda_{2} = \max(g_{d},g_{N_{\rm gen}})/\bar g
= (1/d)/(7/24) = 8/7 \!=\! 1.143$. The empirical ratio,
Symanzik-extrapolated on the canonical ladder, is
\begin{equation}
\lambda_{3}/\lambda_{2}(N\!\to\!\infty) \;=\; 1.010,
\label{eq:phase4a_ratio}
\end{equation}
\emph{falsifying} the Cartesian-product reading. The top six
non-trivial eigenvalues $\lambda_{2},\ldots,\lambda_{7}$ are
near-degenerate with relative spacing $\Delta\lambda/\lambda
\sim 0.01$--$0.05$: the spectrum exhibits a \emph{soft edge}
at the closure value $7/24$ rather than a multiplicative-gap
two-factor structure. The participation ratio of the
$\lambda_{2}$-mode is $\mathrm{PR}\!\approx\!1/N_{\rm gen} = 1/3$
(suggestive of a generation-index mode) and the correlation
of the same mode with node degree vanishes asymptotically
$\rho(\mathrm{mode},\deg)\!\to\!0$ (mode is not degree-tracking).
This rules out the cleanest mechanistic readings (tensor-product
factorisation; degree-localised Fiedler mode) and supports the
Pham--Peron--Metz cavity reading
of Proposition~\ref{prop:gap_statistical_fingerprint} where the
gap edge is set by joint $(\text{degree},\text{triangle-degree})$
statistics, not by individual axis structure. Audit script:
\pyown{src/derive_skeleton_eigenvector_mode_audit.py}{derive_skeleton_eigenvector_mode_audit}.
\end{remark}

\begin{remark}[Analytic-route inventory for the open
$(\mathrm{SG})$ derivation]
\label{rmk:sg_route_inventory}
Beyond the routes surveyed in
\path|notes/lemma_B_proof_strategy.md|, four additional
literature anchors have been screened for direct applicability
to the carrier ensemble at $d_{\rm eff}^{\rm skel}\!=\!12$:
\textbf{(R7)}~Backhausz--Szegedy action-convergence of
operators/graphs and its size-transferability extension
(arXiv:1811.00626; arXiv:2306.04495) covers intermediate-density
graphops and yields a uniformity-over-$N$ lever; it does not pin
the exact rational $3/8$. \textbf{(R8)}~Deterministic recursive
small-world (e.g.~the hierarchical $K_{d}$-replacement networks
of Zhang et al., Sci.~Reports 5:9024, 2015): the specific
construction inspected gives clustering $((d{-}1)/d)^{2}\!=\!9/16$
and spectral dimension $d_{s}\!=\!2$, both mismatched with the
carrier's $\langle C\rangle\!\approx\!0.142$ and
$d_{\rm eff}^{\rm skel}\!=\!12$, but the closed-form spectral
recursion of the construction is a structurally exact template
that may transfer to a carrier-matched variant.
\textbf{(R9)}~Huang--Landon local law for sparse random graphs
(arXiv:1510.06390) requires average degree $p N \!\sim\!N^{\delta}$
with $\delta>0$ growing in $N$, hence is \emph{not} applicable to
the $O(1)$-mean-degree carrier.
\textbf{(R10)}~The Kesten--McKay baseline at fixed $d$
(Bauerschmidt--Huang--Yau \cite{BauerschmidtHuangYau2019},
Remark~\ref{rmk:km_baseline} above), together with a
Bordenave--Lelarge-type free-convolution treatment of the
carrier's empirical degree distribution, is the most concrete
non-cavity analytic anchor; closed-form delivery of the rational
$3/8$ from this anchor would require the irrational
$1 - 2\sqrt{11}/12$ to combine with a structured rational triangle
correction. None of (R7)--(R10) supersedes the Pham--Peron--Metz
cavity route of
Proposition~\ref{prop:gap_statistical_fingerprint}; (R7) and (R9)
in combination yield M3-unconditional uniformity (existence of a
constant gap over $N$) without exact-value derivation, (R8) gives
a deterministic template at the cost of carrier-matching, and
(R10) gives a rigorous baseline within $0.6\%$. The exact-value
analytic frontier remains the cavity calculation of
Proposition~\ref{prop:gap_statistical_fingerprint}.
\end{remark}
...
\pyown{src/verify_lemma_B_radial_hypothesis.py}{verify_lemma_B_radial_hypothesis}
(output
\path|outputs/verify_lemma_B_radial_hypothesis.json|) reports
that the radial-uniform-Poincar\'e hypothesis is not supported
by the data. This is
complementary to the earlier RMT-class audit
\pyown{src/verify_xi_gram_spectral_gap_scaling.py}{verify_xi_gram_spectral_gap_scaling}, which reports
...
$\lambda_{\infty}$ of Proposition~\ref{prop:sg_empirical}
...
\pyown{src/verify_lemma_B_degree_concentration.py}{verify_lemma_B_degree_concentration}
(output \path|outputs/verify_lemma_B_degree_concentration.json|)
reports that the degree-concentration hypothesis is not
supported.
...
expander-graph theory (Alon--Boppana
bounds~\cite{AlonBoppana1986},
Friedman's theorem~\cite{Friedman2008},
Kahale spectral bounds for irregular
expanders~\cite{Kahale1995}),
...
dependent skeleton mean vertex degree
$d_{\rm skel}(\tau,N)\!=\!\mathrm{Mean}_{i}\#\{j:\xi_{N}(i,j)\!>\!\tau\}$
--- a graph \emph{degree} ($\!\approx\!12$ at $\tau\!=\!0.10$),
not to be confused with the \emph{spectral dimension}
$d_{\mathrm{eff}}\!\approx\!3.17$ of the $3{+}1$
Lorentz-signature readout nor with the Ahlfors
\emph{regularity dimension} $d_{*}$ of the continuum-limit
theorem (Theorem~\ref{thm:hard_xi_continuum})
---
exhibits a connectivity hierarchy: $d_{\rm skel}$ saturates at
distinct $N\!\to\!\infty$ asymptotes at each threshold,
\begin{align}
d_{\rm skel}(0.01)\!&\to\!36,\qquad
d_{\rm skel}(0.05)\!\to\!26,\qquad
d_{\rm skel}(0.10)\!\to\!12,\notag\\
d_{\rm skel}(0.20)\!&\to\!5.5,\qquad
d_{\rm skel}(0.50)\!\to\!0.67,\qquad
d_{\rm sig}^{10\%}\!\to\!1.38,
\label{eq:connectivity_hierarchy}
\end{align}
where $d_{\rm sig}^{10\%}$ counts row-significant edges (carrying
$\ge\!10\%$ of the row sum). The Alon--Boppana
bound~\cite{AlonBoppana1986} for the
...
Friedman's theorem~\cite{Friedman2008}: typical random
$d$-regular graphs achieve
...
\pyown{src/verify_lemma_B_edge_weight_structure.py}{verify_lemma_B_edge_weight_structure}
(output
...
expander with $d_{\rm skel}\!=\!12$ asymptotic regular degree,
the uniform spectral gap of
Proposition~\ref{prop:sg_empirical} follows analytically via
Friedman--Kahale-type
bounds~\cite{Friedman2008,Kahale1995}. The conjectured closed form
...
The reproducer
\pyown{src/verify_lemma_B_skeleton_laplacian.py}{verify_lemma_B_skeleton_laplacian}
(output \path|outputs/verify_lemma_B_skeleton_laplacian.json|)
reports both Symanzik-$1$ fits and the cross-seed asymptote
comparison.

\textbf{Conjectures (rational closed forms) --- and a
threshold-sensitivity caveat.}
\begin{equation}
\lambda_{\infty}^{\rm weighted}\!\stackrel{?}{=}\!\tfrac{3}{8}\!=\!0.3750
\quad(\Delta\!=\!1.0\%)
\label{eq:weighted_conjecture}
\end{equation}
is a candidate algebraic identity in
$(d, N_{\rm gen})\!=\!(4, 3)$, $3/8\!=\!(d{-}1)/(2 d)$, on the
\emph{threshold-free} weighted Laplacian. It is robust under
re-sampling and stable across the $10$-regime ladder.
\begin{equation}
\lambda_{\infty}^{\rm skel}(\tau\!=\!0.10)\!=\!0.2924
\quad\text{vs.\ candidate}\quad
\tfrac{7}{24}\!=\!0.29167
\quad(\Delta\!=\!0.24\%)
\label{eq:skeleton_threshold_coincidence}
\end{equation}
is a \emph{threshold-specific coincidence}, not a robust
algebraic identity. The threshold-sweep audit
(\pyown{src/verify_lemma_B_threshold_sweep.py}{verify_lemma_B_threshold_sweep}, output
\path|outputs/verify_lemma_B_threshold_sweep.json|) shows
that $\lambda_{\infty}^{\rm skel}(\tau)$ varies smoothly from
$0.49$ at $\tau\!=\!0.005$ to $0.11$ at $\tau\!=\!0.30$ and
the conjectured $7/24$ value is met only in a narrow window
around $\tau\!=\!0.10$ (where the threshold was originally
chosen because $d_{\rm skel}(\tau\!=\!0.10)\!\approx\!d\cdot
N_{\rm gen}\!=\!12$).
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
...
\toprule
$\tau$ & $\lambda_{\infty}^{\rm skel}(\tau)$ & $d_{\rm skel}^{\infty}(\tau)$ &
$\lambda_{\rm AB}(d_{\rm skel})$ & $|\Delta|$ to $7/24$ \\
\midrule
$0.005$ & $0.492$ & $35.9$ & $0.671$ & $68.8\%$ \\
$0.010$ & $0.492$ & $35.9$ & $0.671$ & $68.7\%$ \\
$0.020$ & $0.488$ & $35.1$ & $0.667$ & $67.2\%$ \\
$0.050$ & $0.425$ & $25.7$ & $0.613$ & $45.6\%$ \\
\textbf{$0.100$} & \textbf{$0.292$} & \textbf{$12.1$} & \textbf{$0.450$} & \textbf{$0.24\%$} \\
$0.200$ & $0.202$ & $5.5$  & $0.228$ & $30.6\%$ \\
$0.300$ & $0.114$ & $3.8$  & $0.121$ & $61.1\%$ \\
\bottomrule
\end{tabular}
\end{center}
Conclusion: the weighted-Laplacian conjecture
\eqref{eq:weighted_conjecture} is robust (threshold-free)
and the present Phase-$2$ leading rational target; the
skeleton-Laplacian rational at $7/24$ is a
\emph{coincidence of threshold choice} and is downgraded
from ``conjecture'' to ``registered finite-$N$ value''. The
ratio $9/7$ derived earlier as
$(3/8)/(7/24)$ is correspondingly downgraded. The Step-$4$
analytical target is the weighted-Laplacian
$3/8\!=\!(d{-}1)/(2 d)$, related to the $\tau\!=\!0.10$
skeleton rational $7/24$ by the exact algebraic identity
$3/8\!=\!(7/24)\cdot(9/7)$.
...
\textbf{Status of the Step-$4$ analytical target.} The two
empirically certified asymptotes --- the threshold-free
weighted-Laplacian gap $\lambda_{\infty}^{\rm weighted}\!=\!3/8$
(Eq.~\eqref{eq:weighted_conjecture}) and the $\tau\!=\!0.10$
skeleton gap $\lambda_{\infty}^{\rm skel}\!=\!7/24$
(Eq.~\eqref{eq:skeleton_threshold_coincidence}) --- satisfy the
exact algebraic identity
\begin{equation}
\tfrac{3}{8}\;=\;\bigl(\tfrac{7}{24}\bigr)\cdot\bigl(\tfrac{9}{7}\bigr)
            \;=\;\tfrac{d-1}{2\,d},
\label{eq:lambda_w_chain}
\end{equation}
with lift factor $9/7\!=\!(3/8)/(7/24)$. This identity is pure
rational arithmetic in the integer inputs
$(d,N_{\rm gen})\!=\!(4,3)$. A \emph{structural} derivation of
the factorisation from the carrier-action equilibrium --- a
mechanism that produces the $7/24$ skeleton gap and the $9/7$
weight-distribution lift, rather than merely recording that
their product is $3/8$ --- remains the open Step-$4$ analytical
deliverable. The leading robust target is the threshold-free
weighted-Laplacian conjecture $3/8\!=\!(d{-}1)/(2d)$ of
Eq.~\eqref{eq:weighted_conjecture}; small-world spectral theory
(Proposition~\ref{prop:edge_correlation_small_world}) is the
working route, and candidate alternative analytical routes are
surveyed in \path|notes/lemma_B_proof_strategy.md|.
\end{proposition}

\begin{proposition}[Skeleton diameter scales logarithmically:
small-world signature, not Cayley-geometric]
\label{prop:skeleton_diameter_friedman}
On the same $10$-regime ladder ($\le 8$ seeds per regime; the
$O(N^{2})$ shortest-path cost limits the seed count at large
$N$), the diameter of the $\tau\!=\!0.10$ skeleton scales
\emph{logarithmically} with $N$:
\begin{equation}
\mathrm{diam}(A_{\rm skel})(N) \;\approx\;
   -0.47 \;+\; 0.84\,\log N,
\label{eq:diameter_log_fit}
\end{equation}
AICc-preferred over the power-law model $c\,N^{\alpha}$ with
$\Delta\mathrm{AICc}\!=\!+3.07$ in favour of the logarithmic
fit. Combined with $100\%$ giant-component fraction on every
regime, girth $\!=\!3$ on every regime, and skeleton-degree
CV $\!\sim\!0.24$ close to the $1/\sqrt{d_{\rm skel}}$
Friedman-random value~\cite{Friedman2008}, the skeleton has
\emph{small-world}-type connectivity: short paths (log-$N$
diameter) on a sparse near-regular random expander base.
The Cayley-graph interpretation is rejected by the
log-$N$ diameter; whether the small-world regime is
Friedman-pure (vanishing clustering) or
Watts--Strogatz-like (persistent clustering) is decided
by the edge-correlation audit
(Proposition~\ref{prop:edge_correlation_small_world}) which
finds the Watts--Strogatz regime applies. The Step-$3$c
isolated-eigenvalue count of
Proposition~\ref{prop:spectral_fingerprint} is re-interpreted
in light of this verdict: the linear-in-$N$ count of
``isolated'' eigenvalues was a MAD-detector artefact reading
fine bulk clustering as discrete peaks, not a Cayley-
representation signature. The skeleton has a Kesten--McKay
bulk with at most $O(1)$ true isolated eigenvalues (the
Perron value and a few near-Perron stragglers) plus
persistent triangle clustering.

Reproducer
\pyown{src/verify_lemma_B_skeleton_diameter.py}{verify_lemma_B_skeleton_diameter}
(output \path|outputs/verify_lemma_B_skeleton_diameter.json|).
\end{proposition}

The Step-$3$d verdict
(Proposition~\ref{prop:skeleton_diameter_friedman}) qualifies
the Phase-$2$ analytical route. The Cayley-graph hypothesis is
rejected by the log-$N$ diameter scaling. The
Friedman-pure-random-regular hypothesis is in turn qualified
by the edge-correlation audit
(Proposition~\ref{prop:edge_correlation_small_world}):
persistent triangle excess (asymptote $5.3\times$ ER baseline)
and persistent clustering coefficient ($\to\!0.142$) refute the
pure-ER edge-formation rule. The combined signature — log-$N$
diameter, girth $\!=\!3$, persistent clustering — is the
classical Watts--Strogatz \emph{small-world graph}
regime~\cite{WattsStrogatz1998}.
The analytical Phase-$2$ route is therefore refined to
small-world spectral theory rather than Friedman direct: the
open analytical target is to show that the carrier-action
equilibrium produces a structured small-world graph ensemble
whose weighted-Laplacian spectral gap saturates at
$3/8\!=\!(d{-}1)/(2d)$ --- equivalently, via the exact
algebraic identity $3/8\!=\!(7/24)\cdot(9/7)$, that the
$\tau\!=\!0.10$ skeleton gap reaches $7/24$ and the
edge-weight distribution supplies the $9/7$ lift. Candidate
analytical routes are surveyed in
\path|notes/lemma_B_proof_strategy.md|.

\begin{proposition}[Skeleton edge-correlation: small-world,
not pure Erd\H{o}s--R\'enyi]
\label{prop:edge_correlation_small_world}
On the $10$-regime ladder ($184$ seeds), the $\tau\!=\!0.10$
skeleton triangle count
$T_{N}\!=\!\mathrm{tr}(A_{\rm skel}^{3})/6$ exceeds the
Erd\H{o}s--R\'enyi expectation
$T_{N}^{\rm ER}\!=\!\binom{N}{3}p_{\rm edge}^{3}$ by a
ratio whose Symanzik-$1$ extrapolation is
\begin{equation}
T_{N}/T_{N}^{\rm ER} \;\to\; 5.31,\qquad
\text{global clustering coefficient }\;
C\;\to\;0.142,
\label{eq:small_world_signature}
\end{equation}
with the edge-inclusion probability decaying as
$p_{\rm edge}(N)\!=\!19/N$ (Symanzik-$1$). Persistent
triangle excess and finite asymptotic clustering, combined
with the log-$N$ diameter scaling of
Proposition~\ref{prop:skeleton_diameter_friedman} and the
girth-$3$ observation of the same proposition, are the
classical Watts--Strogatz \emph{small-world graph}
signature~\cite{WattsStrogatz1998}: short paths plus high
local clustering, distinct from both pure Erd\H{o}s--R\'enyi
(clustering $\to\!0$) and pure regular lattice
(diameter $\sim\!N^{1/k}$). The empirical signature places the
carrier in the \emph{critical regime} between the two
extremes: a pure Erd\H{o}s--R\'enyi graph at the observed
edge density would have $\lambda_{2}\!\to\!1$ but no local
geometry (no admissible $r_{N}$-cover), while a pure regular
lattice at the observed clustering would have
$\lambda_{2}\!\to\!0$ in the continuum limit (diameter
diverges). Watts--Strogatz lies in the narrow band where the
carrier admits both global reachability ($\lambda_{2}$
bounded away from zero) \emph{and} local geometric structure
(persistent clustering); the empirical $\lambda_{\infty}\!=\!3/8$
(vacuum) and $79/200$ (matter) closed-forms are the
quantitative signature of this critical regime.
The analytical Phase-$2$ route
for the target $(\mathrm{SG})$-Lemma is therefore small-world spectral theory, not
Friedman's theorem direct.
Reproducer
\pyown{src/verify_lemma_B_edge_correlation.py}{verify_lemma_B_edge_correlation}, output
\path|outputs/verify_lemma_B_edge_correlation.json|.
\end{proposition}

\begin{proposition}[Statistical fingerprint of the
$(\mathrm{SG})$ gap: degree~$+$~triangle distribution is the
determining statistic]
\label{prop:gap_statistical_fingerprint}
The skeleton-Laplacian asymptote $\lambda_{\infty}^{\rm skel}\!
\to\!7/24$ --- and via Eq.~\eqref{eq:lambda_w_chain} the
weighted-Laplacian conjecture $\lambda_{w}^{\infty}\!=\!3/8$
--- is a \emph{function of the carrier skeleton's graph
statistics alone}, and specifically of the joint (degree,
per-node triangle count) distribution.
A GPU decomposition study
(\pyown{src/verify_lemma_B_gap_statistical_fingerprint.py}{verify_lemma_B_gap_statistical_fingerprint},
output
\path|outputs/verify_lemma_B_gap_statistical_fingerprint.json|;
$\sim\!12.5\,$h on an RTX 5070) generates five synthetic
ensembles at $N$ up to $128\mathrm{k}$ with $250$ seeds per
$(\text{ensemble},N)$ pair, each matching progressively more
of the carrier skeleton statistics, and Symanzik-1
extrapolates the normalised-Laplacian gap $\lambda_{2}(L)$:
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
\begin{tabular}{@{}lcr@{}}
\toprule
Ensemble & matched statistics & $\lambda_{\infty}$ \\
\midrule
E0 Erd\H{o}s--R\'enyi & mean degree only & $0.481$ \\
E1 configuration & degree distribution & $0.481$ \\
\textbf{E2 Newman clustered} & \textbf{degree + per-node triangle count} & $\mathbf{0.293}$ \\
E3 Watts--Strogatz & small-world rewiring at carrier $C$ & $0.173$ \\
E4 random regular & regular at mean degree & $0.485$ \\
\midrule
Carrier ground truth (same script) & --- & $0.293$ \\
\bottomrule
\end{tabular}
\end{center}
\textbf{Only the Newman-clustered ensemble~\cite{Newman2009clustered}
--- the configuration model conditioned on the empirical joint
(independent-edge degree, triangle-degree) sequence ---
reproduces the carrier asymptote}, at $+0.36\%$ relative
residual (vs.\ $+0.36\%$ for the carrier itself relative to
$7/24$); all other ensembles miss by $-40$ to $+66\%$.
A complementary Watts--Strogatz rewiring-probability sweep
(Part~C of the same script, $12$ values $p\!\in\![0,1]$,
$100$ seeds at $N\!\in\![8\mathrm{k},64\mathrm{k}]$) gives
$\lambda_{\infty}^{\rm WS}(p)$ monotone in $p$, crossing
$7/24$ between $p\!=\!0.40$ and $p\!=\!0.55$ but with no
single $p$ within $3\%$: pure small-world rewiring at fixed
mean degree is \emph{not} the determining mechanism.
\textbf{Consequence for the analytical target.} The
structural derivation of
$3/8\!=\!(7/24)\cdot(9/7)$ from the carrier action reduces
to a concrete graph-theoretic statement: the normalised-
Laplacian spectral gap of the Newman-clustered-graph ensemble
with the carrier's empirical (degree, triangle) distribution
equals $7/24$. The cavity / resolvent machinery for sparse
networks with triangles of Pham, Peron and
Metz~\cite{PhamPeronMetz2024} --- separating single-edge
($J_{e}^{2}$) and triangle ($J_{\Delta}^{2}$) contributions to
the diagonal resolvent --- is the precise analytical tool;
small-world spectral theory (Route 5b of
\path|notes/lemma_B_proof_strategy.md|) is thereby
\emph{empirically validated} as the correct analytical route,
and the open Step-$4$ deliverable is the closed-form cavity
solution for the carrier's clustered-degree distribution.
The closed-form high-connectivity-limit resolvent equations of
Silva and Metz~\cite{SilvaMetz2022} reduce the distributional
fixed-point to a single self-consistent equation for the
average resolvent on graphs with arbitrary degree distribution;
extending that closed form to the (degree, triangle) joint case
of~\cite{PhamPeronMetz2024} is the concrete analytical step
expected to deliver $\lambda_{\infty}^{\rm skel}\!=\!7/24$ in
closed form.

\emph{Sketch of the closed-form Step-4 attempt.} The algebraic identity
$7/24\!=\!1/(2d)\!+\!1/(2N_{\rm gen})$ admits a clean
harmonic-sum decomposition into the two integer inputs
$(d,N_{\rm gen})\!=\!(4,3)$. The natural cavity-method
realisation of this decomposition --- a tensor-product
factorisation of the carrier's $(k,k_{\Delta})$ joint
distribution into independent spatial and generation channels
contributing per-channel spectral gaps of $1/(2d)$ and
$1/(2N_{\rm gen})$ --- was tested empirically in
\pyown{src/verify_lemma_B_step4_hsd_factorization.py}{verify_lemma_B_step4_hsd_factorization} (bundled
output \path|outputs/verify_lemma_B_step4_hsd_factorization.json|;
note \path|notes/lemma_B_step4_cavity_sketch_2026_05_15.md|).
The diagnostic is the joint cumulant
$\kappa_{1,1}(N)\!=\!\langle k k_{\Delta}\rangle\!-\!\langle k\rangle
\langle k_{\Delta}\rangle$: under a factorising joint
distribution $\kappa_{1,1}(N)\!\to\!0$ as $N\!\to\!\infty$. The
canonical ladder $N\!\in\![50,512]$ with $8$--$24$ seeds per
regime yields a Symanzik-1 asymptote
$a_{\infty}\!\approx\!+35.4$ (not zero), with the Pearson
correlation $\rho(k,k_{\Delta})\!\approx\!+0.96$ remarkably
stable across the entire ladder. The factorisation hypothesis
is therefore \emph{empirically falsified}: high-degree carrier
nodes are also triangle hubs in a near-perfectly linear
$k_{\Delta}\!\propto\!k$ relation. The mean degree at $N\!=\!512$
($\langle k\rangle\!=\!12.11$) is nonetheless consistent with the
identified
$d_{\rm eff}^{\rm skel}\!=\!d\!\cdot\!N_{\rm gen}\!=\!12$
within seed scatter, supporting that identification. The
closed-form Step-4 derivation therefore requires the full
PPM~\cite{PhamPeronMetz2024} + Silva--Metz~\cite{SilvaMetz2022}
cavity machinery applied to the empirically correlated joint
distribution with $\rho\!\approx\!+0.96$, which is a strongly-
correlated-bivariate-distribution extension genuinely beyond
the open literature; the harmonic-sum identity remains a
necessary algebraic identity the cavity equations must
reproduce, but the route to it via tensor factorisation is
ruled out.
\end{proposition}

\paragraph{Closure entry: carrier-axis-bundle correlation
$\rho(k,k_{\Delta})\!=\!24/25$.}
\label{par:rho_carrier_axis_bundle}
The strongly correlated bivariate structure ruling out the
tensor-product factorisation in
Proposition~\ref{prop:gap_statistical_fingerprint} carries a
sharp structural closure of its own. The empirical Pearson
correlation between node degree and triangle-count on the
$\tau\!=\!0.10$ skeleton, pooled across the $10$ canonical
ladder sizes $N\!\in\![50,512]$ with $8$--$24$ seeds per regime,
is $\rho_{\rm pooled}\!=\!0.9602\!\pm\!0.0034$
($95\%$ CI $[0.957, 0.964]$; per-regime spread only
$\pm 0.01$ between $N\!=\!50$ and $N\!=\!512$). The
small-integer System-$\mathcal R$-rational identification
\begin{equation}
\label{eq:rho_carrier_axis_bundle}
\rho(k,k_{\Delta})\;=\;
\frac{2\,d\,N_{\rm gen}}{(d{+}1)^{2}}
\;=\; 1\;-\;\frac{1}{(d{+}1)^{2}}
\;=\; 1\;-\;(N_{\rm gen}{+}1)\,\gamma^{2}
\;=\;\frac{24}{25}\;=\;0.9600
\end{equation}
sits at $z\!=\!-0.13$ inside the pooled $95\%$ CI; the residual
is $0.02\,\%$, well inside the EXACT tier. The three
equivalent algebraic forms admit three equivalent physical
readings:
(i) the ratio of chirality-spin-times-generation content
($2dN_{\rm gen}\!=\!24$) to the cone-extended bundle area
($(d{+}1)^{2}\!=\!25$);
(ii) the complement of the inverse cone-extended bundle area
$1/(d{+}1)^{2}$;
(iii) the complement of the generation-leakage rate
$(N_{\rm gen}{+}1)\gamma^{2}$ with $\gamma\!=\!1/10$.
The empirical scale-invariance of $\rho$ over $10\times$
range of $N$ identifies the carrier as a strongly-correlated
configuration model with near-functional
$k_{\Delta}\!\propto\!k$ coupling --- the high-degree carrier
nodes are also triangle hubs in a deterministic proportionality
fixed by $(d, N_{\rm gen})$ alone. Universality predictions for
hypothetical alternative integer pairs (currently untestable
but structurally registered): $\rho\!=\!15/16$ for $(d,N_{\rm gen})
\!=\!(3,3)$; $\rho\!=\!35/36$ for $(d,N_{\rm gen})\!=\!(5,3)$.
Reproducer:
\pyown{src/verify_lemma_B_step4_hsd_factorization.py}{verify_lemma_B_step4_hsd_factorization};
bundled output
\path|outputs/verify_lemma_B_step4_hsd_factorization.json|.
...
($\ge 99.8\%$) inside the Kesten--McKay
support~\cite{McKay1981}
...
+ discrete isolated bands''~\cite{McKay1981} is consistent with a
...
\pyown{src/verify_xi_gram_spectral_gap_scaling.py}{verify_xi_gram_spectral_gap_scaling} on the
P$0$--P$8$ ladder (which finds intermediate Poisson--GOE
behaviour). Reproducer
\pyown{src/verify_lemma_B_spectral_fingerprint.py}{verify_lemma_B_spectral_fingerprint}, output
\path|outputs/verify_lemma_B_spectral_fingerprint.json|.
\end{proposition}

\paragraph{Phase-$2$ empirical characterisation: summary.}
Propositions~\ref{prop:sg_empirical}--\ref{prop:spectral_fingerprint}
(plus
Proposition~\ref{prop:skeleton_diameter_friedman}) constitute
the complete empirical characterisation of the carrier
spectral structure relevant to the target $(\mathrm{SG})$-Lemma. The findings are
mutually consistent and lock in the
Phase-$2$ analytical route:
...
\begin{tabular}{@{}lll@{}}
\toprule
Property & Empirical asymptote ($N\!\to\!\infty$) & Reference \\
\midrule
Weighted Laplacian $\lambda_{\infty}$ & $0.3789$ (Symanzik-$1$);
conjectured $3/8$ at $1.0\%$ & Prop.~\ref{prop:sg_empirical} \\
$\xi_{N}$ rank scaling & N-extensive
(low-rank hypothesis falsified) &
Prop.~\ref{prop:radial_hypothesis_falsified} \\
Mean weighted degree & $4.24$ (constant); $\mathrm{CV}\!=\!0.168$
(constant) & Prop.~\ref{prop:degree_concentration_negative} \\
Skeleton structural degree $d_{\rm skel}(\tau\!=\!0.10)$ & $12.14$;
Alon--Boppana $\lambda_{\rm AB}\!=\!0.4499$ &
Prop.~\ref{prop:expander_skeleton} \\
Skeleton Laplacian $\lambda_{\infty}^{\rm skel}$ & $0.2924$;
conjectured $7/24$ at $0.24\%$ &
Prop.~\ref{prop:skeleton_laplacian_audit} \\
Skeleton diameter scaling & $-0.47\!+\!0.84\log N$
(Friedman regime) &
Prop.~\ref{prop:skeleton_diameter_friedman} \\
Skeleton bulk spectrum & inside Kesten--McKay support
$\ge\!99.8\%$ & Prop.~\ref{prop:spectral_fingerprint} \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Per-node-percentile and bulk/matter-core
decomposition (R-loop audit).}
The Phase-$2$ Q-loop already showed that the
skeleton-rational $7/24$ at $\tau\!=\!0.10$ is a
threshold-choice coincidence rather than a robust identity.
The R-loop deepens this by decomposing the weighted
Laplacian per-node-percentile and per-bulk/matter-core
(reproducer
\pyown{src/verify_lemma_B_percentile_decomposition.py}{verify_lemma_B_percentile_decomposition},
output
\path|outputs/verify_lemma_B_percentile_decomposition.json|).
For each per-snapshot, vertices are sorted by row-sum
(matter-core proxy: high row-sum $\Rightarrow$ strongly
coupled $\Rightarrow$ matter-core); the sub-Laplacian of
the top-$p\%$ and bottom-$p\%$ slices is diagonalised and
its Symanzik-$1$ asymptote in $N$ is reported:
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
\begin{tabular}{@{}lll@{}}
\toprule
Cut & Symanzik-$1$ $\lambda_{\infty}$ & Note \\
\midrule
Full (no cut)        & $0.379$ & matches $3/8\!=\!(d{-}1)/(2 d)$, $\Delta\!=\!1.0\%$ \\
Top $50\%$ (matter-core half)  & $0.249$ & $\approx 1/4\!=\!1/d$, $\Delta\!=\!0.4\%$ \\
Top $20\%$           & $0.096$ & sub-Laplacian sparser \\
Top $\le 10\%$       & sub-graph disconnection artefact \\
Bottom $50\%$ (bulk half)      & $0.284$ & $\approx 2/7$, $\Delta\!=\!0.7\%$ \\
Bottom $20\%$        & $0.112$ & sub-Laplacian sparser \\
Bottom $\le 10\%$    & sub-graph disconnection artefact \\
\bottomrule
\end{tabular}
\end{center}
Three observations: (i) the full-Laplacian asymptote
$3/8$ is \emph{not} reproduced by any sub-cut --- it is a
genuinely global property; (ii) bulk-half ($0.284$) and
core-half ($0.249$) values differ by $\sim\!14\%$, so the
carrier has bulk/matter-core spectral inhomogeneity;
(iii) the Fiedler-vector participation ratio is
\emph{extensive but not uniform}: the linear fit
$\mathrm{PR}(N)\!\approx\!0.298\,N\!+\!9.7$ gives
$\mathrm{PR}/N\!\to\!0.31$ (Symanzik-$1$), so about
$30\%$ of vertices are spectrally active in the Fiedler
mode while the remaining $70\%$ contribute negligibly.
The asymptotic fraction $0.31$ sits $4.2\%$ above the
candidate System-$\mathcal R$ rational
$\alpha_{\xi}/N_{\rm gen}\!=\!3/10$; this is suggestive
but outside the corpus EXACT/PRECISE tier thresholds, so
the identification is registered as exploratory.

\paragraph{Where are the spectrally active vertices?}
Two audits address this. The first
(\pyown{src/verify_lemma_B_fiedler_overlap.py}{verify_lemma_B_fiedler_overlap}) uses the
naive row-sum$(\Xi)$ proxy; the second
(\pyown{src/verify_lemma_B_fiedler_vs_corpus_matter_core.py}{verify_lemma_B_fiedler_vs_corpus_matter_core})
uses the corpus-validated row-variance$(\Xi)$ proxy
(reported AUC $0.83$--$0.90$ in $8/8$ LORO folds in the
anisotropic-source companion paper~\cite{Paper4B}) plus
asymmetric overlaps against the C$95$ (top-$5\%$) and C$99$
(top-$1\%$) percentile layers of the corpus-standard
matter-core hierarchy.
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
\begin{tabular}{@{}llll@{}}
...
\midrule
$J$(Fiedler-top$30$, core-top$30$)  & $0.26$ & $0.18$ & $+45\%$ \\
$J$(Fiedler-top$30$, bulk-bot$30$)  & $0.12$ & $0.18$ & $-35\%$ \\
$J$(Fiedler-top$30$, core-top$50$)  & $0.30$ & $0.18$ & $+67\%$ \\
$J$(Fiedler-top$30$, bulk-bot$50$)  & $0.17$ & $0.18$ & $-7\%$ \\
\bottomrule
\end{tabular}
\end{center}
The corpus-validated row-variance proxy yields
$J(\mathrm{Fiedler}_{30\%},\mathrm{rowvar}_{30\%})\!=\!0.28$
($1.6\times$ random baseline $0.18$) and
$J(\mathrm{Fiedler}_{30\%},\mathrm{C99}_{\rm rowvar})\!=\!0.02$--$0.11$
($\sim\!2\times$ random, falling with $N$). The row-sum
proxy gives $J\!=\!0.26$ ($1.45\times$ random).

A third audit
(\pyown{src/verify_lemma_B_fiedler_vs_t00.py}{verify_lemma_B_fiedler_vs_t00}, output
\path|outputs/verify_lemma_B_fiedler_vs_t00.json|) computes
the corpus-canonical matter-core classifier $t_{00}(a)$
directly from $\psi/K/Q$ via \texttt{per\_seed\_galerkin}
(loaded from \pyown{src/verify_galerkin_runner_A_hessian_ricci.py}{verify_galerkin_runner_A_hessian_ricci};
the same pipeline that defines the AUC-$0.85$/$\mathrm{enrichment}\!=\!11.3\times$
classifier reported in P$4$B~\cite{Paper4B}). Six seeds per
regime (Galerkin-throttled), $9$-regime ladder; results
agree qualitatively with both proxies:
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
\begin{tabular}{@{}lccc@{}}
...
\midrule
$J(\mathrm{Fiedler}_{30\%},\,|t_{00}|_{\rm top10\%})$ & $0.137$ & $0.057$ & $2.4$ \\
$J(\mathrm{Fiedler}_{30\%},\,|t_{00}|_{\rm top30\%})$ & $0.249$ & $0.180$ & $1.38$ \\
$J(\mathrm{Fiedler}_{30\%},\,|t_{00}|_{\rm top5\%})$  & $0.078$ & $0.029$ & $2.7$ \\
$J(\mathrm{Fiedler}_{30\%},\,|t_{00}|_{\rm top1\%})$  & $0.039$ & $0.006$ & $6$ \\
$J(\mathrm{rowvar}_{30\%},\,|t_{00}|_{\rm top30\%})$  & $0.430$ & $0.180$ & $2.4$ \\
\bottomrule
\end{tabular}
\end{center}
The picture is consistent across all three proxies
(row-sum, row-variance, canonical $t_{00}$): the
Fiedler-active set correlates with the broader matter-core
\emph{neighbourhood} (top-$10\%$ to top-$30\%$, factor
$1.4$--$2.4$ over random) but \emph{not} with the deep
matter-core C$99$ in absolute terms (only $J\!\sim\!0.04$,
even though the over-random ratio is $\sim\!6\times$
because the random baseline itself is so small). The
row-variance proxy is in turn validated against the
canonical $t_{00}$ classifier at $J\!=\!0.43$
($2.4\times$ random) --- adequate, not perfect. The
Fiedler hot-spots are therefore the \emph{interface
vertices} surrounding the matter-core, not the matter-core
itself. This is consistent with the small-world signature
of Proposition~\ref{prop:edge_correlation_small_world}:
small-world graphs have characteristic interface
populations between clusters that carry the spectral
gap, and standard Cheeger-cut theory makes the Fiedler
vector live on precisely such interface populations. The
$\sim\!30\%$ active fraction is therefore the empirical
size of this interface set in the carrier-action
geometry, not a generic carrier identity.

\paragraph{Halo-test: BFS-distance from the cusp.}
The interface-vs-halo distinction is sharpened by a
direct BFS-distance audit
(\pyown{src/verify_lemma_B_fiedler_halo_test.py}{verify_lemma_B_fiedler_halo_test}): for
each node the graph-distance to the nearest C$99$
matter-core cusp (defined by row-variance$(\Xi)$ top-$1\%$)
is computed on the $\tau\!=\!0.10$ skeleton; the
Fiedler-top-$30\%$ set is binned into ``cusp'' (distance
$0$), ``halo'' (distance $1$--$2$) and ``bulk'' (distance
$\ge\!3$). Across the ladder:
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
...
\end{tabular}
\end{center}
The Fiedler-set is heavily halo-dominated ($75$--$95\%$ in
distance $1$--$2$ from the cusp), but the random baseline
is also halo-dominated (because the skeleton diameter
scales as $\log N$, most of the graph sits within distance
$2$ of any top-$1\%$ subset). The \emph{differential}
signal between $\bar d_{\rm Fiedler}$ and $\bar d_{\rm random}$
is modest ($\sim\!4\%$ at $N\!=\!512$, $\sim\!0\%$ at
$N\!=\!50$). The cleanest reading is therefore:
\begin{itemize}
\item the whole skeleton graph has the NFW-class topology
(cusp $+$ halo $+$ small bulk), the same topology
identified in the P$4$B signed-halo audit~\cite{Paper4B};
\item the Fiedler-active set lives within this halo
topology with a weak positive bias toward the broader
matter-core neighbourhood (Jaccard $0.28$ to row-variance
top-$30\%$, $+58\%$ over random) but is not a sharp
halo-radius identifier;
\item the $\sim\!30\%$ active fraction is the empirical
size of the spectrally-active subset of the halo, not a
generic halo identity.
\end{itemize}
The qualitative picture --- spectral gap localised on the
halo of an NFW-class matter-core, not on the cusp itself
--- is robust and physically consistent with the
P$4$B dark-matter classification.

The top-$50\%$ and bottom-$50\%$ rational candidates
$1/4$ and $2/7$ are registered as exploratory observations,
not as robust conjectures (sub-Laplacian asymptotes depend
on the cut and finite-size corrections grow rapidly as
$p\!\to\!0$).

\begin{proposition}[Defect-condensation hypothesis falsified:
the $(\mathrm{SG})$ is carried by no matter-core or halo
sub-population]
\label{prop:defect_condensation_falsified}
A natural dynamical origin for the $(\mathrm{SG})$ axiom would be
\emph{defect condensation}: the hypothesis that a percolating
sub-network of defect / matter-core nodes carries the spectral
gap, so that $\lambda_{2}$ is a localised sub-population
property. This is tested directly
(\pyown{src/verify_lemma_B_defect_condensation_test.py}{verify_lemma_B_defect_condensation_test}, output
\path|outputs/verify_lemma_B_defect_condensation_test.json|) on
the $10$-regime canonical ladder, $8$ seeds per regime. For each
percentile $p\!\in\!\{1,5,10,25,50\}\%$ the top-$p\%$ nodes of a
candidate sub-population are removed and $\lambda_{2}$ of the
residual weighted Laplacian is compared, as a per-snapshot
$z$-score, against a $20$-permutation random-removal null at the
same node count. Two sub-populations are ranked --- \emph{core}
(top-$p\%$ by row-variance$(\Xi)$, the AUC-$0.83$--$0.90$
matter-core cusp proxy) and \emph{halo} (top-$p\%$ by
$|\text{Fiedler }v_{2}|$) --- branch-resolved into vacuum
($N\!\le\!100$) and matter ($N\!\ge\!256$). The defect-driven
verdict is registered as $z\!<\!-1$ (targeted removal collapses
$\lambda_{2}$ below the random null) at the \emph{deep}
percentiles $p\!\in\!\{10,25,50\}\%$ --- the percentiles at
which a percolating defect network, if present, would manifest
its collapse.

The two rankings show structurally distinct, fully understood
$p$-profiles. The \emph{core} branch-averaged $z$ is flat at
$\approx\!+0.5$ to $+1.6$ across all percentiles and all
regimes: removal of row-variance cusp nodes mildly \emph{raises}
$\lambda_{2}$, with no $p$-dependence (the cusp set is diffuse).
The \emph{halo} branch-averaged $z$ decays monotonically ---
$+3.5$/$+4.9$ at $p\!=\!1\%$ (vacuum/matter), through
$+1.8$/$+2.1$ at $p\!=\!10\%$, to $+0.21$/$-0.33$ at
$p\!=\!50\%$ --- in every one of the ten regimes. This decay is
the expected signature of the $|v_{2}|$ ranking: at small $p$
the removed set is the pair of spectral-bisection poles, whose
excision leaves a residual that is markedly better connected
relative to its own bisection (large positive $z$); as $p$
grows the removal set approaches half the graph and the
operation degenerates --- ``top-$50\%$ by $|v_{2}|$'' and a
random half become structurally indistinguishable --- so $z$
washes out to zero (with a residual sign scatter
$|z|\!\lesssim\!0.5$).

Over the entire verdict domain ($p\!\in\!\{10,25,50\}\%$, both
rankings, all ten regimes) the $z$-score lies in
$[-0.50,\,+2.48]$ --- nowhere near the $z\!<\!-1$ collapse
threshold; the single mildly negative branch-average,
matter-halo at $p\!=\!50\%$ ($z\!=\!-0.33$, a $1.2\%$ decrease),
is the expected $p\!=\!50\%$ halo washout, not a percolation
collapse. All four (branch~$\times$~ranking) verdict groups
return \textsc{drives\_sg}~$=$~\textsc{false}. Both
sub-populations are therefore mild spectral \emph{bottlenecks}
--- their removal raises $\lambda_{2}$ --- not spectral-gap
\emph{carriers}, and the $(\mathrm{SG})$ is a globally
distributed graph property. This is the negative control that
corroborates the extensive participation ratio
$\mathrm{PR}(N)\!\propto\!N$ of
Proposition~\ref{prop:radial_hypothesis_falsified} and excludes
the localisation-based analytical routes (radial low-rank
synthesis; defect condensation) in favour of the small-world
spectral-theory route.
\end{proposition}

\paragraph{Cross-validation against the alt-anchor ladder
(R3).}
The P$0$--P$8$ alt-anchor ladder (used for the
Wigner--Dyson level-spacing audit
\pyown{src/verify_xi_gram_spectral_gap_scaling.py}{verify_xi_gram_spectral_gap_scaling}) catalogs
nine distinct physics regimes at $N\!\in\!\{18,28,30,36,42,50,60,72,84\}$;
each P-anchor is a separate physics setup, not the same
regime at varying $N$, so the cross-regime sequence is
\emph{not} a Symanzik-$1$ continuum-limit ladder. Pointwise
consistency at the overlap with the canonical-physics
ladder: P$5$ canonical and P$5$\_alt at $N\!=\!50$ are
identical (both $\lambda_{2}\!=\!0.515$), and the P$6$/P$7$/P$8$
alt-anchor values at $N\!\in\!\{60,72,84\}$ are within
$1$--$3\%$ of the canonical $\mathcal{P}_{5}N$ values at
the same $N$. The $3/8$ conjecture is therefore a property
of the canonical-physics continuum limit, not a sectoral
identity across alt-anchor regimes. Reproducer
\pyown{src/verify_lemma_B_p0p8_cross_validation.py}{verify_lemma_B_p0p8_cross_validation}.

The locked-in analytical Phase-$2$ programme, after the
threshold-sensitivity audit of
Proposition~\ref{prop:skeleton_laplacian_audit} downgrading
the skeleton-rational $7/24$ to a threshold-specific
coincidence, reduces to a single closed-form target on the
threshold-free weighted Laplacian:
\begin{equation}
\text{(Step~4, weighted-Laplacian asymptote)}\qquad
\lambda_{\infty}^{\rm weighted} \;\stackrel{?}{=}\;
\tfrac{3}{8}\;=\;\tfrac{d-1}{2\,d},
\label{eq:step4_target}
\end{equation}
algebraic in the integer inputs
$(d, N_{\rm gen})\!=\!(4, 3)$ and threshold-free. The
empirical Symanzik-$1$ asymptote $0.3789$ matches at $1.0\%$
relative residual. The analytical proof, derivation of
\eqref{eq:step4_target} from the carrier-action equilibrium
construction, is the remaining Phase-$2$ deliverable; it
would resolve the conditional structure of the master
closure theorem of P6~\cite{Paper6} (the emergent Einstein
equation conditional on A1--A8) into an unconditional
theorem. Three explicit falsification triggers for
\eqref{eq:step4_target} are registered in
\path|notes/lemma_B_proof_strategy.md| (FB-w$1$/$2$/$3$); the
deciding one is FB-w$1$: cross-seed mean
$\lambda_{2}(L_{w})$ outside $[0.365,0.385]$ on an extended
ladder $N\!\ge\!1024$.

\paragraph{Relation to the wider 3-Lemma decomposition.}
The initial Phase-$2$ framing introduced three target lemmas:
\emph{Lemma~A} (uniform doubling A8 on admissible carriers),
\emph{Lemma~B} (this section's $(\mathrm{SG})$ uniform
spectral-gap target), and \emph{Lemma~C} (operator/tensor
convergence $G_{N}\!\to\!G_{\infty}$,
$T_{N}^{\Xi}\!\to\!T_{\infty}^{\Xi}$ as distributions on
the mm-GH limit). The current status is:
\textbf{Lemma~A} is \emph{subsumed} by Lemma~B
(Theorem~\ref{thm:sg_implies_admissibility} above shows
$(\mathrm{SG})$ implies the full admissibility set
A1+A8 via uniform Poincar\'e plus Cheeger--Buser), so no
separate Lemma-A analytical attack is required;
\textbf{Lemma~B} is the open analytical target
(empirically characterised; the leading robust target is the
threshold-free weighted-Laplacian conjecture
$3/8\!=\!(d\!-\!1)/(2d)$, with the $\tau\!=\!0.10$ skeleton gap
$7/24$ and the lift factor $9/7\!=\!(3/8)/(7/24)$ related by the
exact algebraic identity $3/8\!=\!(7/24)\cdot(9/7)$; a
structural derivation of this factorisation from the carrier
action remains open, see
Proposition~\ref{prop:skeleton_laplacian_audit} and the route
survey in \path|notes/lemma_B_proof_strategy.md|);
\textbf{Lemma~C} is \emph{numerically certified} by the
$5$-proxy Dal~Maso $\Gamma$-convergence audit
($\gamma$-score $0.849$ in the
\texttt{GAMMA\_CONVERGED} band, master-synthesis chapter
of P6~\cite{Paper6} and reproducer
\pyown{src/verify_clp_c_gamma_convergence_detailed.py}{verify_clp_c_gamma_convergence_detailed}), with
the analytical replacement of the numerical proxies
remaining open but supporting the conditional master
closure theorem without further analytical work in the
present session. A session-level
scoping memo is at
\path|notes/lemma_B_step4_analytical_attempt.md|: it
verifies the algebraic identities
$7/24\!=\!(d{+}N_{\rm gen})/(2 d N_{\rm gen})$ and
$3/8\!=\!(d{-}1)/(2 d)$ as algebraic consequences of the
integer inputs $(d, N_{\rm gen})\!=\!(4, 3)$, identifies the
skeleton-degree identity $d_{\rm skel}^{\infty}\!=\!d \cdot
N_{\rm gen}\!=\!12$, and scopes the remaining open Step-$4$
deliverable --- a structural derivation of the factorisation
$3/8\!=\!(7/24)\cdot(9/7)$ from the carrier-action
equilibrium --- with small-world spectral theory the working
route and candidate alternatives surveyed in
\path|notes/lemma_B_proof_strategy.md|.
...
(\pyown{src/verify_clp_c_gamma_convergence_detailed.py}{verify_clp_c_gamma_convergence_detailed},
\pyown{src/stage6f_fine_percentile_audit.py}{stage6f_fine_percentile_audit},
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_continuum_bianchi.py}{verify_continuum_bianchi},
\pyown{src/verify_lambda_extrapolation_discipline_audit.py}{verify_lambda_extrapolation_discipline_audit}).
...
applicability:
\pyown{src/verify_cone_tightness_extended_ladder.py}{verify_cone_tightness_extended_ladder} reports on
...
across model points;
\pyown{src/verify_cone_tightness_p5_ladder.py}{verify_cone_tightness_p5_ladder} reports on the
...
\pyown{src/verify_gap_monotonicity_p5_ladder.py}{verify_gap_monotonicity_p5_ladder} (output
...
\pyown{src/verify_causal_wave_structural_significance.py}{verify_causal_wave_structural_significance}) gives
...
The CBI thus ties together three otherwise independent
...
(\pyown{src/stage6h_matter_core_phase_diagram.py}{stage6h_matter_core_phase_diagram},
\pyown{src/stage6h_core_geometric_identification.py}{stage6h_core_geometric_identification};
bundles
...
(reproducer
\pyown{src/stage6h_layer_modes_v2.py}{stage6h_layer_modes_v2}; bundle
...
(reproducer
\pyown{src/stage6h_layer_shell_structure_test.py}{stage6h_layer_shell_structure_test}; bundle
...
(reproducer
\pyown{src/verify_full_tensor_alpha_family.py}{verify_full_tensor_alpha_family}, output
...
\paragraph{Statistical status of Theorem~\ref{thm:percentile_spectrum_law}
(disclosure).} The $\hat\alpha_{\rho}$ in the boxed display are
log--log slope estimates \emph{measured} on the residual ladder;
they are an empirical regression and their $95\%$ bootstrap CIs
are wide (e.g.\ $\hat\alpha_{p_{90}}=0.922\,[0.622,2.245]$). The
four classical anchor rationals
($\mathrm{med}\!=\!1/3$, $\mathrm{mean}\!=\!17/20$,
$p_{99}\!=\!1$, $\sup\!=\!1/2$) are \emph{not} read off from
this regression --- they are \emph{independently derived} from
the spectral geometry of the relational graph in
Remark~\ref{rmk:sketch_four_rationals} (two from
framework-internal lemmas, e.g.\ $17/20\!=\!\alpha_{\xi}\!+\!
\varepsilon_{\rm sync}^{2}\!-\!\gamma$ as the non-scalar
Clifford-channel reaction rate; two from standard
concentration-of-measure bounds). The role of the empirical
regression here is as a \emph{consistency check} of those
independent structural derivations: each derived rational lies
inside its bootstrap CI. Because the CIs are wide, that check
is statistically weak --- it does not by itself \emph{prove}
the rationals --- but the rationals stand on
Remark~\ref{rmk:sketch_four_rationals}, not on the regression.
The CI-robust content of the theorem proper is the qualitative
\emph{rising-then-falling profile} and the strictly-positive
lower CI bound on the ten bulk percentiles.
...
the framework, each empirically validated on the twelve-point
Federer ladder $N\!\in\![50,300]$ (regimes P5, P6, P5N64, P7,
P5N72, P8, P5N84, P5N100, P5N128, P5N200, P5N256, P5N300;
extension to $N\!=\!512$ remains the next compute step):
...
of \eqref{eq:lambda-trace-asymptotic} is not as a global
symmetry-broken form; the trace
$\sum_\mu\Lambda^\mu_{\;\mu}$ is branch-resolved at
$\theta_{\rm chir}\!=\!\pi/4$ into
$\Lambda_{t}^{\rm vacuum}\!=\!33/40$ and
$\Lambda_{t}^{\rm matter}\!=\!81/100$ per
Eq.~\eqref{eq:lambda_t_branch_resolved}.
...
available evidence in the present analysis.
...
fields are positive and consistent across all ten regimes
...
$72$ (24)   & $+0.36\;[+0.15,+0.55]$ & $+0.42\;[+0.21,+0.60]$ \\
$84$ (24)   & $+0.37\;[+0.17,+0.54]$ & $+0.42\;[+0.23,+0.58]$ \\
$100$ (24)  & $+0.39\;[+0.21,+0.54]$ & $+0.41\;[+0.23,+0.57]$ \\
$128$ (12)  & $+0.43\;[+0.31,+0.58]$ & $+0.33\;[+0.18,+0.49]$ \\
$200$ (8)   & $+0.41\;[+0.29,+0.52]$ & $+0.35\;[+0.23,+0.47]$ \\
$256$ (12)  & $+0.39\;[+0.27,+0.48]$ & $+0.34\;[+0.22,+0.45]$ \\
$300$ (12)  & $+0.39\;[+0.29,+0.48]$ & $+0.38\;[+0.26,+0.46]$ \\
$512$ (12)  & $+0.34\;[+0.27,+0.42]$ & $+0.26\;[+0.17,+0.34]$ \\
\hline
\end{tabular}
\end{center}
The bracketed numbers are bootstrap $95\%$ CIs on the seed-mean
$\rho_{\rm S}$. All ten bootstrap CIs exclude zero with margin
for both $T_{00}$ and $G_{00}$. The mean values
$\langle\rho_{\rm S}(\mathrm{EC},T_{00})\rangle = +0.39$ and
$\langle\rho_{\rm S}(\mathrm{EC},G_{00})\rangle = +0.37$ are
stable across the regime ladder, with regime-to-regime spread
$\le 0.10$ on each correlation. Eigenvector
...
the mean degree per bin yields the top-vs-bottom decile
$\deg(\text{top})/\!\deg(\text{bottom})$ ratios
\begin{center}
\begin{tabular}{rcc}
\hline
$N$ & $\deg(\text{top})/\deg(\text{bot})$ & permutation null $97.5\%$ \\
\hline
$50$  & $3.74$ & $1.64$ \\
$64$  & $5.08$ & $1.75$ \\
$72$  & $4.40$ & $1.65$ \\
...
\end{tabular}
\end{center}
The observed top-vs-bottom ratio sits well above the
permutation-null $97.5\%$ quantile at every regime
($N$-grand-mean ratio $7.5$, null $97.5\%$ mean $1.55$),
i.e.\ degree-concentration on $T_{00}$-top nodes is highly
significant against random $T_{00}$ shuffles across the full
ten-regime ladder. The ratio grows monotonically with $N$
because the $\Xi$-graph thins (edge density $0.040\!\to\!0.0035$
across the ladder) faster in the low-$T_{00}$ tails than at the
matter-core. Together with (i) this confirms that high-$T_{00}$
vertices preferentially occupy high-connectivity positions in
$\mathcal G_\Xi$.
...
properties on $\mathcal G_\Xi$ are stable across the
canonical-physics ladder $N\in\{50,64,72,84,100,128,200,256,
300,512\}$ scanned at full seed coverage:
\emph{(i)} stress-energy and Galerkin curvature are both
positively rank-correlated with eigenvector centrality
($\rho_{\rm S}\approx +0.39$ for $T_{00}$ and $+0.37$ for
$G_{00}$, with bootstrap $95\%$ CIs excluding zero in every
regime for both fields);
\emph{(ii)} matter-core nodes carry a top-vs-bottom decile
degree ratio well above the permutation-null $97.5\%$
quantile in every regime ($N$-grand-mean $7.5$ vs null
$1.55$);
...
is exactly forest-like across the canonical regimes scanned:
$H_1=0$ and $0$--$1$ triangles per regime. The full
$\mathcal G_\Xi$ is also nearly acyclic, with a regime-mean of
$4.5,1.0,1.0$ cycles at $N=50,64,100$. Random-subset null
distributions matched to core size are likewise zero, consistent
with the underlying $\mathcal G_\Xi$ being topologically
forest-like at the canonical coupling scale rather than this
being a property specific to matter localisation.

The persistence diagram is informative beyond the
single-threshold reading. Tracking the $H_1$ birth times in the
extended filtration range $\varepsilon\in[\varepsilon^{\ast},3]$
yields a regime-stable mean birth filtration
\begin{equation*}
\bar\varepsilon_{H_1}(N) \;\in\; \{1.91, 2.12, 2.15\}
\quad\text{for}\quad N\in\{50, 64, 100\},
\end{equation*}
...
is $N$-stable, suggesting that the critical coupling $\Xi_c$ is
a structural property of the canonical regime rather than a
finite-$N$ artefact, modulo the seed coverage (8 seeds per
regime for $N\!\in\!\{50,64,100,128,200,256\}$; extension to
the full 12-seed $N\!\in\!\{256,300,512\}$ runs at full
persistent-homology resolution is the next compute step, as the
$\mathcal{O}(N^{2})$ filtration cost grows steeply with $N$).
Reproducer:
\pyown{src/analyze_xi_filtration_complete.py}{analyze_xi_filtration_complete}
with bundle
\path|outputs/xi_filtration_complete.json|.
...
$n_{s} = 1 - \gamma^{2}(d{+}N_{\rm gen})/2 = 193/200 = 0.965$
(equivalently $1 - (d{+}N_{\rm gen})\,\gamma\,\varepsilon_{\rm sync}^{2}$
using $\varepsilon_{\rm sync}^{2}=\gamma/2$), matching the Planck~2018
best-fit ($n_{s} = 0.9649 \pm 0.0042$) within $0.02\sigma$; \emph{(b)} a dynamical
...
$r_{\mathrm{pred}} = 0.029$ sits a factor of $\sim 1.2$ below the
most recent end-of-2025 Planck--SPT--ACT--BICEP/Keck--DESI 95\%
upper bound $r < 0.034$~\cite{Balkenhol2025} (a factor $\sim 1.9$
below the earlier BK18 stand-alone bound
...
while the observed Planck-2018~\cite{Planck2018} vacuum
energy density is
...
handle.  Reproducer:
\pyown{src/stage6b_solve_4_problems.py}{stage6b_solve_4_problems}; bundle
...
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/_d1_npz_discovery.py}{_d1_npz_discovery}:
\pyown{src/analyze_xi_graph_topology_deep.py}{analyze_xi_graph_topology_deep} (eigenvector
centrality, degree concentration, Betti-$H_{1}$ check) writing
\path|outputs/xi_graph_topology_deep.json|;
\pyown{src/analyze_extended_cluster_diagnostics.py}{analyze_extended_cluster_diagnostics} (Laplacian
...
\pyown{src/analyze_xi_filtration_complete.py}{analyze_xi_filtration_complete} (Vietoris--Rips
...
\item This paper does not itself derive a UV completion of
      ...
      companion absorbs the UV-completion entry that would
      otherwise stand as an open item here.
...
\item {\sloppy Discrete-graph topology of $T_{00}$ and $G_{00}$
      ...
      (reproducer
      \pyown{src/verify_p5n512_topology_extension.py}{verify_p5n512_topology_extension}).\par}
...
Reproducer
\pyown{src/verify_heavy_tail_lead_lag_p5n512.py}{verify_heavy_tail_lead_lag_p5n512}, output
...
\pyown{src/verify_xi_eom_chirality_flip_bounce.py}{verify_xi_eom_chirality_flip_bounce}, output
...
\pyown{src/verify_O5_spatial_running_lambda_p5n512.py}{verify_O5_spatial_running_lambda_p5n512},
output
...
\appendix
\input{density_contrast_law_derivation}
...
Causal sets \cite{BombelliSorkin1987,Sorkin2003,Surya2019} replace
the continuum with a locally finite partial order. Both programs
share fundamental discreteness as a UV regulator and a relational
primitive, but our construction uses a weighted graph with
similarity $\Xi_{ij}$ on edges (not a partial order) and derives
the Einstein tensor via Galerkin Hessian-Ricci projection rather
than via counting causal-relation cardinalities. The two are not
isomorphic; the comparison point is that both predict
ultraviolet-regulated continuum dynamics with discrete substrate.

\paragraph{Gravity from entropy (Bianconi 2025).}
The most direct sibling of the present construction is the
entropic-action framework of Bianconi~\cite{Bianconi2025}, in
which the gravitational action is itself the quantum relative
entropy between the spacetime metric and a metric induced by
the matter fields and the geometry, reducing in the low-coupling
regime to a dressed Einstein--Hilbert action with an emergent
\emph{small positive} cosmological constant and a $G$-field
dark-matter candidate. The structural parallel is close, and on several axes closer
than a first reading suggests. In both frameworks the Einstein
equation and a positive $\Lambda$-term are \emph{read off} an
entropy/structure functional (the quantum relative entropy
there, the relational correlation functional $\Xi$ here) rather
than postulated; both carry an emergent Lorentzian metric (the
$3{+}1$ causal layer of Section~\ref{sec:emergence} here); both
describe matter through a Dirac--K\"ahler / staggered
topological content (Becher--Joos~\cite{BecherJoos1982} and the
Kogut--Susskind staggered Dirac operator of the companion
chirality paper~\cite{Paper4D} here); and both deliver an
emergent positive cosmological-constant term
($\Lambda_{\mu\nu}^{\mathrm{back}}$ here, Section~\ref{sec:cbi})
together with a dark-sector candidate. The genuine distinction
is therefore not the metric signature or the matter content but
the substrate of the variational principle: Bianconi works with
a \emph{continuum} action that is literally the quantum relative
entropy between two metrics, whereas the present construction
reads the Einstein closure off a \emph{finite relational
$\Xi$-graph} via Galerkin Hessian-Ricci projection and per-node
residual minimisation. The two are members of the same
entropy/information-functional emergent-gravity direction, and
the corpus is best read as a discrete, parameter-counted
instance of that direction rather than in isolation.

\paragraph{Spacetime from entanglement (Van Raamsdonk, Swingle).}
A further contact point is the ``entanglement builds geometry''
programme: Van Raamsdonk~\cite{VanRaamsdonk2010} argued that the
connectivity of a classical spacetime is built from the quantum
entanglement of the underlying degrees of freedom, and
Swingle~\cite{Swingle2018} reviewed the tensor-network
realisation in which a discrete geometry encoding entanglement
obeys Einstein's equations under a continuum-limit assumption.
The present construction is the same move with a different
primitive: the emergent metric $d_{ij}=-\ell_{n}\log\Xi_{ij}$ is
read off the relational \emph{correlation} functional $\Xi$, the
discrete analogue of building geometry from a correlation/
entanglement structure, and the continuum-limit assumption is
made explicit and audited (Theorem~\ref{thm:hard_xi_continuum})
rather than left implicit.

\paragraph{Graph-spectral emergent geometry.}
A directly kindred recent direction reads geometry off the
\emph{spectrum} of a discrete graph operator: a partition
function summing over graphs weighted by their spectral data,
with an Einstein--Hilbert action evaluated on the resulting
spectral curve and graph Laplacians converging in spectral
distribution to a Laplace--Beltrami operator
\cite{GraphLaplacianString2026}; and an emergent graph Einstein
equation exhibited in p-adic CFT tensor networks
\cite{pAdicTensorEinstein2021}, where the graph curvature is
fixed by the \emph{consistency} of the emergent Einstein
equation rather than postulated. The present construction is
the same spectral-to-geometric move on a relational
$\Xi$-graph --- the emergent metric and the Hessian--Ricci
tensor are read off $\Xi$ and its Galerkin spectral data, the
continuum limit being the mm-GH theorem
(Theorem~\ref{thm:hard_xi_continuum}) --- with the
distinguishing feature that the closure inputs are the two
integers $(d,N_{\rm gen})$, not a tunable action.
...
G.~Bianconi,
\emph{Gravity from entropy},
Physical Review D \textbf{111}, 066001 (2025);
arXiv:2408.14391;
\href{https://doi.org/10.1103/PhysRevD.111.066001}{doi:10.1103/PhysRevD.111.066001}.
Gravitational action defined as the quantum relative entropy
between the metric and a matter/geometry-induced metric;
reduces to a dressed Einstein--Hilbert action with an emergent
small positive cosmological constant and a $G$-field
dark-matter candidate.

\bibitem{VanRaamsdonk2010}
M.~Van Raamsdonk,
\emph{Building up spacetime with quantum entanglement},
General Relativity and Gravitation \textbf{42}, 2323 (2010)
[International Journal of Modern Physics D \textbf{19}, 2429
(2010)]; arXiv:1005.3035.
Connectivity of classical spacetime built from the quantum
entanglement of the underlying degrees of freedom.

\bibitem{Swingle2018}
B.~Swingle,
\emph{Spacetime from entanglement},
Annual Review of Condensed Matter Physics \textbf{9}, 345
(2018).
Review of the tensor-network programme: a discrete geometry
encoding entanglement which, under a continuum-limit
assumption, obeys Einstein's equations.

\bibitem{Surya2019}
S.~Surya,
\emph{The causal set approach to quantum gravity},
Living Reviews in Relativity \textbf{22}, 5 (2019);
arXiv:1903.11544.
Contemporary living review of spacetime emerging from a
discrete causal order.

\bibitem{Surya2024fluctuations}
S.~Surya \textit{et al.},
\emph{Fluctuations and correlations in causal set theory},
arXiv:2407.03395 (2024).
Updates Sorkin's everpresent-$\Lambda$ prediction
$\Lambda\!\sim\!1/\sqrt{V_{\rm spacetime}}$ to the discrete
fluctuating regime; closest external analogue to the present
framework's dynamically-selected lattice cosmological
constant $\Lambda_{\rm lat}^{\infty}\!=\!19/15$, because in
both constructions $\Lambda$ is selected by a discrete
spacetime measure rather than tuned by an effective action.

\bibitem{GraphLaplacianString2026}
V.~Tishkov,
\emph{From graph Laplacians to string partition functions: a
rigorous pathway from discrete spectra to emergent geometry},
arXiv:2605.00452 (2026).
Partition function over graphs weighted by their spectral data,
with an Einstein--Hilbert action on the spectral curve; graph
Laplacians converge in spectral distribution to a
Laplace--Beltrami operator.

\bibitem{pAdicTensorEinstein2021}
L.~Chen, X.~Liu, L.-Y.~Hung,
\emph{Emergent Einstein equation in p-adic CFT tensor networks},
Physical Review Letters \textbf{127}, 221602 (2021);
arXiv:2102.12022.
Graph curvature fixed by the consistency of an emergent graph
Einstein equation rather than postulated.
...
\bibitem{Bermudez2025}
M.~Bermudez,
\emph{Spontaneous emergence of Lorentzian signature from
curvature-minimizing geometry},
arXiv:2510.07891 (2025).
A curvature-minimising internal symmetric tensor field whose
eigenvalue pattern fixes the local causal structure, yielding
a common origin for flatness and Lorentzian signature --- the
recognised-mechanism precedent for the present
Einstein-residual closure reading the $3{+}1$ structure off
the equilibrium $\Xi$-field.

\bibitem{Newman2009clustered}
M.~E.~J.~Newman,
\emph{Random graphs with clustering},
Physical Review Letters \textbf{103}, 058701 (2009);
arXiv:0903.4009.
The configuration model conditioned on a joint
(independent-edge degree, triangle-degree) sequence; the
synthetic ensemble whose Symanzik asymptote reproduces the
carrier skeleton-Laplacian gap at $7/24$ in
Proposition~\ref{prop:gap_statistical_fingerprint}.

\bibitem{PhamPeronMetz2024}
T.~M.~Pham, T.~Peron, and F.~L.~Metz,
\emph{Effects of clustering heterogeneity on the spectral
density of sparse networks},
Physical Review E \textbf{110}, 054307 (2024);
arXiv:2404.08152.
Self-consistent cavity / resolvent equations for sparse
networks with single edges and triangles, separating
$J_{e}^{2}$ and $J_{\Delta}^{2}$ contributions to the
diagonal resolvent; the precise analytical tool for the
empirically-validated Newman-clustered-graph route to
$\lambda_{\infty}^{\rm skel}\!=\!7/24$.

\bibitem{SilvaMetz2022}
J.~D.~Silva and F.~L.~Metz,
\emph{Analytic solution of the resolvent equations for
heterogeneous random graphs: spectral and localization
properties},
Journal of Physics: Complexity \textbf{3}, 045012 (2022);
arXiv:2209.06805.
Closed-form solution of the cavity resolvent equations in the
high-connectivity limit for graphs with arbitrary degree
distribution; reduces the distributional fixed-point to a
single self-consistent equation for the average resolvent.
The analytical machinery one extends to the (degree, triangle)
joint case of~\cite{PhamPeronMetz2024} to obtain the
closed-form $\lambda_{\infty}^{\rm skel}\!=\!7/24$ targeted
as the Step-4 deliverable of Lemma~B.

\bibitem{BauerschmidtHuangYau2019}
R.~Bauerschmidt, J.~Huang, and H.-T.~Yau,
\emph{Local Kesten--McKay law for random regular graphs},
Communications in Mathematical Physics \textbf{369}, 523 (2019);
arXiv:1609.09052.
Optimal local law: the empirical spectral distribution of a
random $d$-regular graph follows the Kesten--McKay density
$\rho_{\rm KM}(\lambda) = \frac{d}{2\pi}\frac{\sqrt{4(d{-}1) - \lambda^{2}}}{d^{2} - \lambda^{2}}$
on $|\lambda| \le 2\sqrt{d{-}1}$ down to the optimal scale
$N^{-1+\epsilon}$. The rigorous baseline against which the
carrier-skeleton gap is compared in
Remark~\ref{rmk:km_baseline}: the $d{=}12$ Kesten--McKay
upper-edge gap $\lambda_{2}^{\rm KM}(12) = 1 - 2\sqrt{11}/12$
sits within $0.6\%$ of the empirical $\lambda_{\infty}^{\rm skel}$
after subtracting the universal $\gamma^{2}(d{+}N_{\rm gen})$
leakage.

\bibitem{KaloperPadilla2014}
N.~Kaloper and A.~Padilla,
\emph{Vacuum energy sequestering: the framework and its
cosmological consequences},
Physical Review D \textbf{90}, 084023 (2014);
arXiv:1406.0711.
The named ``vacuum-energy sequestering'' framework: a global
constraint prevents the quantum vacuum energy from sourcing
the cosmological constant --- the literature precedent for the
Coleman--Gamow vacuum-sequestering layer of the nine-layer
...
arXiv:2512.10613 (December 2025). Combined
Planck--SPT--ACT--BICEP/Keck analysis with DESI BAO, yielding
$n_s = 0.9682 \pm 0.0032$ (CMB-only) and the 95\% C.L. upper
bound $r < 0.034$.
...
preceded by the short announcement
...
\bibitem{AlonBoppana1986}
N.~Alon,
\emph{Eigenvalues and expanders},
Combinatorica \textbf{6}, 83 (1986).
The Alon--Boppana lower bound on the second eigenvalue of
$d$-regular graphs:
$\lambda_{2}(A)\ge 2\sqrt{d-1}-o(1)$, equivalently
$\lambda_{2}(L_{\rm norm})\le 1-2\sqrt{d-1}/d+o(1)$.
Used in the Phase-$2$ $(\mathrm{SG})$-Lemma audit.

\bibitem{Friedman2008}
J.~Friedman,
\emph{A Proof of Alon's Second Eigenvalue Conjecture and
Related Problems},
Memoirs of the American Mathematical Society \textbf{195}, 910
(2008).
Theorem that random $d$-regular graphs are near-Ramanujan with
high probability: $\lambda_{2}\le 2\sqrt{d-1}+\epsilon$ for any
$\epsilon>0$ as $N\to\infty$. Used to interpret the
$84\%$ Alon--Boppana saturation observed for the carrier
skeleton in
Proposition~\ref{prop:expander_skeleton}.

\bibitem{Kahale1995}
N.~Kahale,
\emph{Eigenvalues and expansion of regular graphs},
Journal of the ACM \textbf{42}, 1091 (1995).
Spectral-gap bounds for irregular weighted expanders;
referenced in the Phase-$2$ Step-$4$b programme for the
$9/7$ weight-lift factor.

\bibitem{McKay1981}
B.~D.~McKay,
\emph{The expected eigenvalue distribution of a large regular
graph},
Linear Algebra and its Applications \textbf{40}, 203 (1981).
Kesten--McKay bulk density for the adjacency spectrum of
random $d$-regular graphs; referenced in the spectral-
fingerprint audit of
Proposition~\ref{prop:spectral_fingerprint}.

\bibitem{WattsStrogatz1998}
D.~J.~Watts and S.~H.~Strogatz,
\emph{Collective dynamics of `small-world' networks},
Nature \textbf{393}, 440 (1998).
Watts--Strogatz small-world model: short paths combined with
high local clustering. The empirical skeleton signature of
Proposition~\ref{prop:edge_correlation_small_world} matches
this regime.
...
P.~van der Hoorn, W.~J.~Cunningham, G.~Lippner, C.~Trugenberger,
and D.~Krioukov,
\emph{Ollivier-Ricci curvature convergence in random geometric
graphs},
Physical Review Research \textbf{3}, 013211 (2021);
arXiv:2008.01209.
Rigorous link between a graph/network discrete curvature and
the continuum Ricci curvature of the underlying manifold; the
external precedent for the discrete-to-continuum curvature
witnesses used here.
