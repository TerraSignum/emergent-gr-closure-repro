...
A 2026-05-13 architectural-ceiling audit
(\path|src/verify_clp_b_architectural_ceiling.py|,
...
\label{thm:cbi}
\emph{Conditional statement.} Assume the two empirical-asymptote
Lemmas~\ref{lem:t00-asymptote} and~\ref{lem:lambda-t-asymptote}
hold --- i.e. that the Symanzik~$2$ extrapolations of
$T_{00}^{\Xi}(N)$ and $\Lambda_{t}(N)$ both converge to the
$\mathcal R$ rational $\alpha_{\xi}^{2}$ in the continuum limit
(these are numerical extrapolations, $R^{2}=0.54$ and the
$\Lambda_{t}$ counterpart, \emph{not} closed-form analytic
limits; see the per-Lemma disclosures). Then, under the
$\mathcal R$ canonical coefficient choice and
...
implication}: its antecedents are the two empirical-asymptote
Lemmas, which are numerical Symanzik extrapolations
($R^{2}=0.54$ for the $T_{00}^{\Xi}$ leg) rather than
closed-form analytic limits. The deduction
``both legs $\to\alpha_{\xi}^{2}$ $\Rightarrow$ ratio $\to 1$''
is exact algebra; what is empirical, and therefore the
load-bearing assumption, is that each leg \emph{does} converge
to $\alpha_{\xi}^{2}$. The CBI should be read at PRECISE tier
(the $0.85\%$ relative offset of the $T_{00}^{\Xi}$
extrapolation), not as a strict-EXACT analytic identity; the
within-$\mathcal R$ algebra is exact, the physical convergence
is the conditional input.
...
bound $CD(K,N)$ (Sturm~\cite{Sturm2006}) / the Lott--Villani
measure-contraction property~\cite{LottVillani2009}
& (A5)$+$(A6)
& \textsc{open implication}. (A5) gives only
$\Delta_{\rm curv}\!\to\!0$ --- the curvature \emph{stabilises}
--- which is \emph{not} a uniform lower bound $K$; (A6) fixes
$N\!=\!d_*$ but not $K$. Deriving $CD(K,N)$ or the
measure-contraction property from (A5)$+$(A6) is the one
load-bearing implication this sketch does \emph{not} close. \\
\bottomrule
\end{tabular}
\end{center}}
...
the single open implication: this is precisely \emph{why} the
result is stated as ``[Sketch]'' and used downstream only in the
conditionally-rigorous sense of
Corollary~\ref{cor:cbi_admissibility}. Promotion to a full
``[Proof]'' requires closing the (A5)$+$(A6)$\,\Rightarrow\,CD(K,N)$
row --- e.g.\ by strengthening (A5) from curvature-\emph{stability}
to a uniform curvature \emph{lower bound}, or by an independent
discrete-to-continuum measure-contraction argument.
\end{proof}
...
master closure theorem along its corresponding axis; replacing
...
$\gamma$-score $0.849$ certifies the conditional master
closure theorem at $95\%$ bootstrap confidence on the
...
above form the CLP-C axis of the master closure-limit-proof
...
is \path|src/verify_lemma_B_matter_branch_universality.py|
(output
...
(\path|src/verify_lemma_B_threshold_sweep.py|, output
...
chosen because $d_{\rm eff}(\tau\!=\!0.10)\!\approx\!d\cdot
N_{\rm gen}\!=\!12$).
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
...
\toprule
$\tau$ & $\lambda_{\infty}^{\rm skel}(\tau)$ & $d_{\rm eff}^{\infty}(\tau)$ &
$\lambda_{\rm AB}(d_{\rm eff})$ & $|\Delta|$ to $7/24$ \\
\midrule
...
$(3/8)/(7/24)$ is correspondingly downgraded. The Step-$4$a
analytical target is now the weighted-Laplacian
$3/8\!=\!(d{-}1)/(2 d)$ alone, not the two-stage Step-4a
+Step-4b decomposition.
...
\textbf{Step-$4$a structural identification (Cartesian-product
synthesis).} The skeleton asymptote $7/24$ admits the clean
algebraic factorisation
\begin{equation}
\tfrac{7}{24}
\;=\;\tfrac{d+N_{\rm gen}}{2\,d\,N_{\rm gen}}
\;=\;\tfrac{1}{2}\Bigl(\tfrac{1}{d}\!+\!\tfrac{1}{N_{\rm gen}}\Bigr),
\label{eq:skel_cartesian_product}
\end{equation}
i.e.\ the arithmetic mean of the inverse spacetime dimension
$1/d$ and the inverse generation count $1/N_{\rm gen}$. The
empirical asymptote on the $10$-regime ladder is
$\lambda_{2}^{\infty}\!=\!0.29238$ at $0.24\%$ relative
deviation from $7/24\!=\!0.29167$ (reproducer
\path|src/verify_lemma_B_carrier_spectral_synthesis.py|,
output \path|outputs/verify_lemma_B_carrier_spectral_synthesis.json|).
This factorisation is the signature of a Cartesian-product
small-world ensemble whose two factors have normalised-Laplacian
spectral gaps $1/d$ (spatial factor) and $1/N_{\rm gen}$
(family factor) respectively. The structural correspondences are:
\begin{itemize}
\item \textbf{Spatial factor:} lazy Markov chain on $d$
spacetime directions with stay probability
$\alpha_{S}\!=\!1-(d-1)/d^{2}\!=\!13/16$ and uniform
per-alternative jump $1/d^{2}\!=\!1/16$. The $1/d^{2}$
per-slot rate is the natural Cl$(1,3)$ spinor-dimension
weighting (at $d\!=\!4$, $d^{2}\!=\!2^{d}\!=\!16$ coincide).
\item \textbf{Family factor:} lazy Markov chain on
$N_{\rm gen}$ fermion generations with stay
$\alpha_{F}\!=\!1-(N_{\rm gen}\!-\!1)/N_{\rm gen}^{2}\!=\!7/9$
and uniform per-alternative jump $1/N_{\rm gen}^{2}\!=\!1/9$.
The $1/N_{\rm gen}^{2}$ per-slot rate is the natural PMNS-
matrix-size weighting (the framework's family-mixing sector
uses $N_{\rm gen}^{2}\!=\!9$ slots for ordered family pairs).
\end{itemize}
The framework anchor for both per-slot rates is the
$\beta_{\pi}$ refined-vacuum closure form
$\beta_{\pi}\!=\!(2^{d}\,N_{\rm gen}^{2}\!-\!1)/(2^{d}\,N_{\rm
gen}^{2})\!=\!143/144$ in the causal-wave companion
paper~\cite{Paper2}, whose denominator
$2^{d}\,N_{\rm gen}^{2}\!=\!144\!=\!(d\,N_{\rm gen})^{2}$
is the squared skeleton mean degree $d_{\rm eff}^{2}$. The two-step
analytical programme is therefore: (i) prove the skeleton
admits the Cartesian-product decomposition into a
spatial factor and a family factor, each with the specified
lazy-Markov spectral gap (Step~$4$a), and (ii) prove the
carrier-action edge-weight distribution lifts the unweighted-
Laplacian gap by the rational factor
$9/7\!=\!(d\!-\!1)N_{\rm gen}/(d\!+\!N_{\rm gen})$
(Step~$4$b, Kahale-type bound).

\textbf{Family-coupling subspace gap = $7/6$ and spatial dilution
factor $1/d$.} A direct empirical test of the family-coupling
matrix's spectral gap on the canonical $d_{1}\,\mathcal{P}_{5}/
\mathcal{P}_{5}N$ ladder (9 regimes, $N\!\in\![64,512]$,
164 seeds) constructs the $3\!\times\!3$ family-coupling matrix
$M_{F}$ by projecting the equilibrium $\Xi$-matrix onto three
generation-level basis vectors (each averaging over three
sectors quark / lepton / higher-mode = $N_{\rm gen}^{2}\!=\!9$
PMNS slot count) and computes its normalised-Laplacian spectral
gap. Symanzik-1 asymptote on the 9 P5/P5N regimes:
\begin{equation}
\lambda_{2}^{\rm family\text{-}coupling}(N\!\to\!\infty) \;=\; \tfrac{7}{6}
\;=\; \tfrac{d+N_{\rm gen}}{2\,N_{\rm gen}},
\label{eq:family_coupling_gap}
\end{equation}
empirical $1.170$, rel.\ err.\ $+0.28\%$ vs $7/6\!=\!1.1667$
(PRECISE tier). \textbf{Master algebraic identity connecting all Lemma~B spectral
anchors.} The family-coupling spectral gap admits the exact
decomposition (Fraction-arithmetic verified)
\begin{equation}
\boxed{\;\lambda_{2}^{\rm family\text{-}coupling}
 \;=\; \alpha_{\xi}\cdot\lambda_{\rm Kahale}
 \;+\; \gamma^{2}\,\cdot\,\left(1\!-\!\frac{1}{N_{\rm gen}(d+N_{\rm gen})}\right)\;}
\label{eq:master_identity_lambda_family}
\end{equation}
that is, $7/6 = (9/10)\cdot(9/7) + (1/100)\cdot(20/21)
= 81/70 + 1/105 = 245/210 = 7/6$ EXACT, where
$20/21\!=\!(N_{\rm gen}(d+N_{\rm gen})\!-\!1)/(N_{\rm gen}(d+N_{\rm gen}))$
follows the structural $(X\!-\!1)/X$ pattern characteristic of
$\gamma^{2}$-scale carrier-action corrections (with $X$ a
framework-defined sub-sector slot count). The same pattern
appears in the refined-vacuum $\beta_{\pi}$ closure of the
causal-wave companion paper~\cite{Paper2},
$\beta_{\pi}\!=\!(2^{d}N_{\rm gen}^{2}\!-\!1)/(2^{d}N_{\rm gen}^{2})\!=\!143/144$,
where the slot count $X\!=\!2^{d}N_{\rm gen}^{2}\!=\!144$ is the
Cl$(1,3)$ Clifford spinor dimension times the PMNS-matrix
squared; for the family-coupling correction the slot count
$X\!=\!N_{\rm gen}(d+N_{\rm gen})\!=\!21$ is the family
generations times the combined dimensional content. The
master identity Eq.~\eqref{eq:master_identity_lambda_family}
is therefore the natural family-sector analogue of the
spinor-sector $\beta_{\pi}$ identity. Under the framework's
structural relation $d\!=\!N_{\rm gen}\!+\!1$, all Lemma-B
spectral anchors simplify to $N_{\rm gen}$-only forms
($\lambda_{2}^{\rm family\text{-}coupling}\!=\!(2N_{\rm gen}\!+\!1)/(2N_{\rm gen})$,
$\lambda_{\rm Kahale}\!=\!N_{\rm gen}^{2}/(2N_{\rm gen}\!+\!1)$,
$\gamma\!=\!1/(2(N_{\rm gen}\!+\!2))$,
$\lambda_{w}^{\rm vac}\!=\!N_{\rm gen}/(2(N_{\rm gen}\!+\!1))$);
the master identity itself is an algebraic coincidence specific
to our framework's parameter values $(d, N_{\rm gen})\!=\!(4, 3)$
where the particular $\gamma\!=\!1/10$ aligns the $\gamma^{2}$-loop
correction with the leading $\alpha_{\xi}\!\cdot\!\lambda_{\rm Kahale}$
term to give EXACT closure. The structural mechanism behind this
alignment from the carrier-action $S_{\Xi}$ remains the final
open analytical step.
This connects ALL the spectral anchors via the System-$\mathcal{R}$
rationals $(\gamma, \alpha_{\xi}, \lambda_{\rm Kahale}\!=\!9/7)$
plus the structural correction $20/21 = d(d+1)/(N_{\rm gen}(d+N_{\rm gen}))$.
The vacuum-branch closure follows by exact algebra:
\begin{align*}
\lambda_{\rm skel}^{\rm vac} &= (1/d)\cdot\lambda_{2}^{\rm family\text{-}coupling}
= (1/4)\cdot(7/6) = 7/24, \\
\lambda_{w}^{\rm vac} &= \lambda_{\rm Kahale}\cdot\lambda_{\rm skel}^{\rm vac}
= (9/7)\cdot(7/24) = (d-1)/(2d) = 3/8\quad\text{EXACT closure},
\end{align*}
and the matter-branch follows by the $\gamma^{2}$-universal
$\lambda_{w}^{\rm mat}\!=\!\lambda_{w}^{\rm vac}\!+\!2\gamma^{2}
\!=\!3/8\!+\!1/50\!=\!79/200$ EXACT. Reproducer:
\path|src/verify_lemma_B_alpha_xi_master_identity.py|.

The family-coupling spectrum is empirically certified as
\emph{branch-invariant} at the $N\!\to\!\infty$ asymptote: both
the vacuum branch (pre-flip regimes $N\!<\!128$) and the matter
branch (post-flip regimes $N\!\geq\!128$) converge to
$\lambda_{2}^{\rm family\text{-}coupling}\!=\!7/6\!=\!(d\!+\!
N_{\rm gen})/(2\,N_{\rm gen})$. The $+2\gamma^{2}$ matter-branch
shift that appears in the skeleton $\lambda_{w}^{\rm mat}\!=\!79/200$
is therefore localised in the \emph{spatial} sector of the carrier-
action (the $1/d$-dilution mechanism), not in the family-mixing
matrix $M_{F}$ itself.

The third normalised-Laplacian eigenvalue
$\lambda_{3}^{\rm family\text{-}coupling}$ is empirically
certified at PRECISE tier to
$11/6\!=\!1.8333$ (Symanzik-1 asymptote $1.830$,
rel.\ err.\ $-0.17\%$); the full spectrum is therefore
$\text{spec}(L_{\rm norm}(M_{F}))\!=\!(0, 7/6, 11/6)$, a
symmetric split of the $K_{3}$ degenerate pair $(0, 3/2, 3/2)$
by $\pm(N_{\rm gen}\!-\!1)/(2\,N_{\rm gen})\!=\!\pm 1/3$ --- a
pure System-$\mathcal{R}$ rational with no $\gamma$-scale
corrections. The same $3\!\times\!3$ family-coupling matrix
structure appears in the SYE fermion-mass extraction
pipeline (\path|src/worldformula/physics/spectral_yukawa_
eigenvalue.py|), which uses the SVD singular values
$\sigma_{i}$ of the analogous $M_{d}^{3\times 3}$
charged-lepton mass operator to extract fermion mass
hierarchies via $m_{i}\!=\!\sigma_{i}\,v_{\rm EW}/\sqrt{2}$;
the two pipelines share the upstream
$\Xi$-to-$9$-mode projection but diverge at the eigenvalue
extraction step (singular values for mass hierarchies versus
normalised-Laplacian eigenvalues for the family-mixing
spectral relaxation rates of this audit). This admits a clean structural relation to the
skeleton spectral gap of
Eq.~\eqref{eq:skel_cartesian_product}:
\begin{equation}
\frac{\lambda_{2}^{\rm skel}(N\!\to\!\infty)}
     {\lambda_{2}^{\rm family\text{-}coupling}(N\!\to\!\infty)}
\;=\; \frac{7/24}{7/6} \;=\; \frac{1}{d} \;=\; \frac{1}{4}\qquad
\text{(EXACT algebraic ratio)}.
\label{eq:spatial_dilution_factor}
\end{equation}
The factor $1/d$ is the spatial-averaging dilution: projecting
the $N$-vertex skeleton adjacency onto the $3$-dim family-
coupling subspace consolidates $N$ lattice nodes per family
generation into a single per-generation mode, enhancing the
spectral gap by exactly factor $d$. Reproducer:
\path|src/verify_lemma_B_family_factor_p5n_canonical.py|,
output
\path|outputs/verify_lemma_B_family_factor_p5n_canonical.json|.

The off-diagonal triple $(\rho_{12},\rho_{13},\rho_{23})$ of the
normalised adjacency $D^{-1/2}|M_{F}|D^{-1/2}$ is constrained by
the spectrum~$(0, 7/6, 11/6)$ via two clean structural invariants:
\begin{equation}
\sum_{i<j}\rho_{ij}^{2} \;=\; 1 - \sigma, \qquad
\prod_{i<j}\rho_{ij} \;=\; \sigma/2,
\qquad
\sigma \;\equiv\; \tfrac{d+1}{N_{\rm gen}\,d\,(d-1)}.
\label{eq:MF_off_diagonal_invariants}
\end{equation}
At $(d,N_{\rm gen})\!=\!(4,3)$ this gives
$\sigma\!=\!5/36$, sum-of-squares $31/36$ and product $5/72$
EXACT. Empirical extraction of the sorted triple on the
canonical~$10$-regime ladder ($152$ pooled seeds) confirms the
arithmetic-progression structural form $(a\!-\!b,a,a\!+\!b)$ with
midpoint-consistency $1.47\%$, while the geometric-progression
candidate $(1/6,1/2,5/6)$ is rejected ($\rho_{\min}\!+35.7\%$).
Under the arith-prog ansatz Eq.~\eqref{eq:MF_off_diagonal_invariants}
reduces to the universal cubic
\begin{equation}
5\,a^{3} \;-\; (1\!-\!\sigma)\,a \;-\; \sigma \;=\; 0,
\label{eq:MF_arith_prog_cubic}
\end{equation}
giving at $(4,3)$ the cubic $180\,a^{3}\!-\!31\,a\!-\!5\!=\!0$
with unique positive real root $a^{*}\!\approx\!0.4797$. The
off-diagonal triple is therefore an algebraic-irrational of
degree~$3$ over~$\mathbb{Q}(\sigma)$, NOT a System-$\mathcal{R}$
rational --- the simplest structural identification compatible
with the empirical extraction. Reproducer:
\path|src/verify_lemma_B_M_F_empirical_off_diagonal_extraction.py|.

\textbf{The Cartesian-product synthesis is therefore replaced
by the direct identification}
\begin{equation}
\lambda_{2}^{\rm skel} \;=\; \tfrac{1}{d}\,\lambda_{2}^{\rm
family\text{-}coupling}
\;=\; \tfrac{1}{d}\,\cdot\,\tfrac{d+N_{\rm gen}}{2\,N_{\rm gen}}
\;=\; \tfrac{d+N_{\rm gen}}{2\,d\,N_{\rm gen}}
\;=\; \tfrac{7}{24},
\label{eq:skel_from_family}
\end{equation}
which supersedes the failed
$(1/d+1/N_{\rm gen})/2$ Cartesian-product framing (the latter is
an algebraic coincidence of the same value, but the literal
per-factor interpretation as separate spatial and family
sub-graphs with gaps $1/d$ and $1/N_{\rm gen}$ is not directly
supported empirically: a parallel test on the existing P1-P5
family-phase microscopic data finds the family-coupling
$\lambda_{2}\!\approx\!1.21$ rather than $1/N_{\rm gen}\!=\!1/3$,
falsifying the literal-factor reading,
\path|src/verify_lemma_B_family_factor_p1p2prime.py|).
The structural picture is therefore: a single
\emph{family-coupling subspace} of dimension $N_{\rm gen}\!=\!3$
with intrinsic spectral gap $(d\!+\!N_{\rm gen})/(2\,N_{\rm gen})$,
diluted by the spatial averaging factor $1/d$ when realised on
the full $N$-vertex skeleton. The algebraic chain
\begin{equation}
\tfrac{3}{8}\;=\;\bigl(\tfrac{7}{24}\bigr)\cdot\bigl(\tfrac{9}{7}\bigr)
            \;=\;\tfrac{d\!-\!1}{2\,d}
\label{eq:lambda_w_chain}
\end{equation}
closes the weighted-Laplacian asymptote exactly. The status is
therefore: the algebraic factorisation is empirically certified
at PRECISE tier on the canonical skeleton, the Kahale lift
factor is empirically certified at PRECISE tier, and the
algebraic chain $3/8 = (7/24)\cdot(9/7) = (d\!-\!1)/(2d)$ is
exact; the structural mechanism by which the carrier-action
equilibrium produces this specific factorisation, however,
remains open and is not a literal product of two factor graphs
with the algebraic decomposition gaps. Session-level memo:
\path|notes/lemma_B_step4a_carrier_synthesis_derivation.md|.
\end{proposition}
...
CV $\!\sim\!0.24$ close to the $1/\sqrt{d_{\rm eff}}$
...
by the Step-$4$a pre-flight audit
...
Reproducer
\path|src/verify_lemma_B_skeleton_diameter.py|
...
small-world spectral theory rather than Friedman direct;
Step~$4$a (the analytical proof of
$\lambda_{\infty}^{\rm skel}\!=\!7/24$ from the carrier
construction) reduces to: show that the carrier-action
equilibrium produces a structured small-world graph
ensemble whose spectral gap (computable via small-world
spectral resolvent methods) saturates at the conjectured
rational. Step~$4$b (the $9/7$ lift factor from the weight
...
Reproducer
\path|src/verify_lemma_B_edge_correlation.py|, output
...
Skeleton structural degree $d_{\rm eff}(\tau\!=\!0.10)$ & $12.14$;
...
(reproducer
\path|src/verify_lemma_B_percentile_decomposition.py|,
output
...
(\path|src/verify_lemma_B_fiedler_overlap.py|) uses the
naive row-sum$(\Xi)$ proxy; the second
(\path|src/verify_lemma_B_fiedler_vs_corpus_matter_core.py|)
...
A third audit
(\path|src/verify_lemma_B_fiedler_vs_t00.py|, output
...
(loaded from \path|src/verify_galerkin_runner_A_hessian_ricci.py|;
...
(\path|src/verify_lemma_B_fiedler_halo_test.py|): for
...
\path|src/verify_xi_gram_spectral_gap_scaling.py|) catalogs
...
\path|src/verify_lemma_B_p0p8_cross_validation.py|.
...
(empirically characterised; Step-$4$a Cartesian-product
synthesis reduces the broad question to two sharper per-factor
structural identifications, see
Proposition~\ref{prop:skeleton_diameter_friedman} and memo
\path|notes/lemma_B_step4a_carrier_synthesis_derivation.md|;
calibration targets $7/24\!=\!(1/d\!+\!1/N_{\rm gen})/2$ and
$3/8\!=\!(d\!-\!1)/(2d)$ algebraically chained via the Kahale
lift $9/7\!=\!(d\!-\!1)N_{\rm gen}/(d\!+\!N_{\rm gen})$);
...
\path|src/verify_clp_c_gamma_convergence_detailed.py|), with
...
skeleton-degree identity $d_{\rm eff}^{\infty}\!=\!d \cdot
N_{\rm gen}\!=\!12$, and scopes the remaining carrier-action
derivation as multi-month research (small-world spectral
synthesis for Step~$4$a, Kahale-type weight lift for
Step~$4$b).
...
(disclosure).} This is an \emph{empirical-regression} result, not
an analytic derivation: the $\hat\alpha_{\rho}$ are
log--log slope estimates fitted to the residual ladder, and
their $95\%$ bootstrap CIs are wide --- e.g.\
$\hat\alpha_{p_{90}}=0.922\,[0.622,2.245]$ has an upper bound
$3.6\times$ the central value. The four classical anchor
rationals ($\mathrm{med}\!\sim\!1/3$,
$\mathrm{mean}\!\sim\!17/20$, $p_{99}\!\sim\!1$,
$\sup\!\sim\!1/2$) are stated as \emph{consistent with} the
fitted exponents (each lies inside its bootstrap CI), \emph{not}
as values the framework derives or predicts a priori; with CIs
this wide the consistency is a weak test. The load-bearing,
CI-robust content of the theorem is the qualitative
\emph{rising-then-falling profile} and the strictly-positive
lower CI bound on the ten bulk percentiles --- the specific
rational identifications are a suggestive overlay at PRECISE
tier, not a strict-EXACT closure.
