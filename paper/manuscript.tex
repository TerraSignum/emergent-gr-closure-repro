...
Friedman-like, not Cayley-geometric]
...
Friedman-random value~\cite{Friedman2008}, the skeleton is
in the \emph{Friedman regime}: a sparse near-regular random
expander, \emph{not} a geometric Cayley graph on a finite
group.

The Step-$3$c isolated-eigenvalue count of
...
representation signature. The skeleton is Friedman-random
through and through, with Kesten--McKay bulk and at most
$O(1)$ true isolated eigenvalues (the Perron value and a few
near-Perron stragglers).
...
two closed-form sub-bounds:
\begin{align}
\text{(Step~4a, Friedman type)}\qquad
\lambda_{\infty}^{\rm skel} \;&=\;
\tfrac{7}{24}\;=\;\tfrac{d{+}N_{\rm gen}}{d\,N_{\rm gen}\!\cdot 2},
\label{eq:step4a_target}\\
\text{(Step~4b, Kahale-type lift)}\qquad
\lambda_{\infty}^{\rm weighted}/\lambda_{\infty}^{\rm skel} \;&=\;
\tfrac{9}{7}\;=\;\tfrac{\alpha_{\xi}^{2}\cdot d}{d\!+\!N_{\rm gen}},
\label{eq:step4b_target}
\end{align}
each addressable independently. The product of
\eqref{eq:step4a_target} and \eqref{eq:step4b_target} is the
weighted-Laplacian asymptote
$\lambda_{\infty}^{\rm weighted}\!=\!3/8\!=\!\alpha_{\xi}^{2}/2\cdot 5/(d{+}1)$
(if both rational conjectures hold), reproducing
Proposition~\ref{prop:sg_empirical}. The full algebraic
identification of $\lambda_{\infty}^{\rm weighted}$ in
\eqref{eq:step4b_target} is a consequence of $\alpha_{\xi}\!=\!9/10$
together with $d\!=\!4$ and $N_{\rm gen}\!=\!3$; it is part of
the calibration target, not an independent algebraic claim.
The Step~$4$ analytical derivation of either rational closes
the corresponding empirical sub-bound; both together resolve
the conditional structure of the master closure theorem of
