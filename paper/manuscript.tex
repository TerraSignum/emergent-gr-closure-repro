...
Compared with the previous broad multi-month framing
``prove the skeleton is a near-Ramanujan expander with
$d_{\rm eff}\!=\!12$'', the Cartesian-product
\emph{algebraic factorisation} reduces Step~$4$a from a
single broad question to a sharper algebraic
question: identify the structural mechanism on the carrier-
action equilibrium graph that produces a normalised-Laplacian
spectral gap matching the arithmetic mean of $1/d$ and
$1/N_{\rm gen}$. The natural per-factor interpretation as
literal spatial and family factor graphs with gaps $1/d$ and
$1/N_{\rm gen}$ respectively is \emph{not} directly supported
by the existing P1-P5 family-phase microscopic data; the
GFS-style family-coupling matrix on that dataset
($136$ seeds, lattice size $28$) has normalised-Laplacian
spectral gap $\approx 1.21$, regime-stable across P1-P5,
which is close to the $K_{3}$ baseline $3/2$ with a
$(1\!+\!\gamma)^{2}$ symmetry-breaking signature rather than
the conjectured $1/N_{\rm gen}\!=\!1/3$
(\path|src/verify_lemma_B_family_factor_p1p2prime.py|,
\path|outputs/verify_lemma_B_family_factor_p1p2prime.json|).
The algebraic chain
\begin{equation}
