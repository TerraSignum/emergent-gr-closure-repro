...
Friedman-random value, the skeleton is in the
\emph{Friedman regime}: a sparse near-regular random
...
(Proposition~\ref{prop:skeleton_diameter_friedman}) locks the
Phase-$2$ analytical route to Friedman's theorem for typical
sparse near-regular random expanders. The Cayley-graph
hypothesis is rejected. Step~$4$a (the analytical proof of
$\lambda_{\infty}^{\rm skel}\!=\!7/24$ from the carrier
construction) reduces to: show that the carrier-action
equilibrium produces a $d\!\approx\!12$-regular random graph
ensemble whose Friedman spectral gap saturates at the
conjectured rational. Step~$4$b (the $9/7$ lift factor from
the weight distribution) is unchanged.
