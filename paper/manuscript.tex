...
$d_{\rm eff}\!=\!12$'', the Cartesian-product synthesis
reduces Step~$4$a to two sharper per-factor structural
questions (spatial gap $=1/d$ from Cl$(1,3)$ spinor
weighting; family gap $=1/N_{\rm gen}$ from PMNS-matrix
weighting). The algebraic chain
\begin{equation}
\tfrac{3}{8}\;=\;\bigl(\tfrac{7}{24}\bigr)\cdot\bigl(\tfrac{9}{7}\bigr)
            \;=\;\tfrac{d\!-\!1}{2\,d}
\label{eq:lambda_w_chain}
\end{equation}
closes the weighted-Laplacian asymptote exactly, modulo the
two per-factor structural identifications. Each of these is
session-tauglich, not multi-month, since the factor graphs
are finite (size $d$ and $N_{\rm gen}$ respectively) and the
lazy-Markov rates are determined by the framework's existing
structural denominators $2^{d}$ and $N_{\rm gen}^{2}$.
Memo: \path|notes/lemma_B_step4a_carrier_synthesis_derivation.md|.
\end{proposition}
