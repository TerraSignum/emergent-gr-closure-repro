...
The follow-up audit
(\path|src/verify_lemma_B_fiedler_overlap.py|, output
\path|outputs/verify_lemma_B_fiedler_overlap.json|)
computes Jaccard overlap between the Fiedler-amplitude
top-$30\%$ set and the row-sum-induced
matter-core/bulk classification:
\begin{center}
\renewcommand{\arraystretch}{1.10}
\small
\begin{tabular}{@{}llll@{}}
...
\end{tabular}
\end{center}
The spectrally active set is \emph{not} identical to the
matter-core set (Jaccard well below $0.5$): it correlates
positively with matter-core ($+45\%$ over random for the
strong test, $+67\%$ for the weaker test) and negatively
with bulk ($-35\%$ over random for the strong test, $-7\%$
for the weaker test). The interpretation is that the
Fiedler hot-spots are the \emph{interface vertices}
between matter-core and bulk, not the matter-core itself.
This is consistent with the small-world signature of
Proposition~\ref{prop:edge_correlation_small_world}:
small-world graphs have characteristic interface
populations between clusters that carry the spectral
gap. The conjectured $\sim\!30\%$ active fraction is
therefore the empirical size of this interface set in
the carrier-action geometry, not a generic carrier
identity.
