...
The locked-in analytical Phase-$2$ programme consists of
two closed-form sub-bounds, each algebraic in the integer
inputs $(d, N_{\rm gen})\!=\!(4, 3)$:
\begin{align}
\text{(Step~4a, small-world spectral theory)}\qquad
\lambda_{\infty}^{\rm skel} \;&\stackrel{?}{=}\;
\tfrac{7}{24}\;=\;\tfrac{d{+}N_{\rm gen}}{2\,d\,N_{\rm gen}},
\label{eq:step4a_target}\\
\text{(Step~4b, weight-distribution lift)}\qquad
\lambda_{\infty}^{\rm weighted} \;&\stackrel{?}{=}\;
\tfrac{3}{8}\;=\;\tfrac{d-1}{2\,d}.
\label{eq:step4b_target}
\end{align}
The product of these is consistent with
the empirical lift factor
$\lambda_{\infty}^{\rm weighted}/\lambda_{\infty}^{\rm skel}\!=\!9/7$.
The structural skeleton has asymptotic degree
$d_{\rm eff}\!=\!d\!\cdot\!N_{\rm gen}\!=\!12$
(Proposition~\ref{prop:expander_skeleton}); the
Alon--Boppana bound at $d\!=\!12$ is
$\lambda_{\rm AB}\!=\!1\!-\!2\sqrt{11}/12\!\approx\!0.4499$, so
the conjectured \eqref{eq:step4a_target} corresponds to
saturation ratio $\sim\!0.65$ (well inside the Friedman regime
but not at the Ramanujan optimum). Both
\eqref{eq:step4a_target} and \eqref{eq:step4b_target} are
\emph{conjectured} closed forms, not analytically derived.
The Step~$4$ analytical work, derivation from the carrier-
action equilibrium construction, is the remaining Phase-$2$
deliverable; both together would resolve the conditional
structure of the master closure theorem of
P6~\cite{Paper6} (the emergent Einstein equation conditional
on A1--A8) into an unconditional theorem.
